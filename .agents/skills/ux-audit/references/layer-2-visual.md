...
- **Captured via the acceptance process** — `agent-browser --cdp 9222 screenshot` renders from the
  renderer (works headless under `xvfb-run`). See `.agents/acceptance/PROCESS.md` for launch +
...
- **Verify before citing.** Open every screenshot with the **Read tool** and confirm it
  shows what you claim _before_ writing a finding — same rule the acceptance process uses for
