...
import { type AgentGroupConfig, ToolNameResolver } from '@orvilo/context-engine';
import { consumeStreamUntilDone, ModelEmptyError } from '@orvilo/model-runtime';
...
// @orvilo/model-runtime resolves to @cloud/business-model-runtime which has
// cloud-specific dependencies that are unavailable in the test environment
vi.mock('@orvilo/model-runtime', async () => {
  // ModelEmptyError + isEmptyModelCompletion are pure (they only depend on
  // @orvilo/types), so import the real implementations directly from source —
...
describe('RuntimeExecutors', { timeout: 60_000 }, () => {
  ...
  describe('stream error detection in call_llm', () => {
    ...
    it('should retry and eventually throw when LLM stream contains error events from provider', async () => {
      ...
      // Import real implementations directly from source (bypassing the @orvilo/model-runtime mock)
