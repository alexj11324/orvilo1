---
title: Customizing Provider Model List in Orvilo for Deployment
description: >-
  Learn how to customize the model list in Orvilo for deployment with the
...
Orvilo supports customizing the model list during deployment. This configuration is done in the environment for each [model provider](/docs/self-hosting/environment-variables/model-provider).
...
docker run -d -p 3210:3210 \
  ...
  --name orvilo-test alexj11324/orvilo1
```
...
- Check logs: `docker logs orvilo | grep -i "model"`
...
- Confirm the model actually supports the capability in the provider's API (Orvilo cannot enable capabilities the API doesn't provide)
- Verify you are running a recent enough version of Orvilo
