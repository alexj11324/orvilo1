import { createSource } from '@orvilo/agent-signal';
import type {
  ...
  it('writes record marker receipt and context accumulator for completed memory outcomes', async () => {
    ...
    const source = createSource({
      payload: {
        ...
        tool: { apiName: 'addPreferenceMemory', identifier: 'orvilo-user-memory' },
      },
  ...
  it('records failed outcomes without handled markers', async () => {
    ...
    const source = createSource({
      payload: {
        ...
        tool: { apiName: 'importFromMarket', identifier: 'orvilo-skill-store' },
      },
      scope: { topicId: 't1', userId: 'u1' },
      scopeKey: 'topic:t1',
      sourceId: 'source_2',
