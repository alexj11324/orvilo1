...
import { dynamicInterventionAudits } from '@orvilo/builtin-tools/dynamicInterventionAudits';
...
import {
  ...
  SpanStatusCode,
  trace as otelTrace,
} from '@orvilo/observability-otel/api';
import {
  ...
  async startExecution(params: StartExecutionParams): Promise<StartExecutionResult> {
    ...
    try {
      ...
      if (!executionContext) {
        ...
        // Note: AgentRuntimeContext requires sessionId for compatibility with @orvilo/agent-runtime
        executionContext = {
          payload: {
            isFirstMessage: true,
            message: [{ content: '' }],
