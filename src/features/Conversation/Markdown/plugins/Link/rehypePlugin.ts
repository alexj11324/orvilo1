...
import { ORVILO_LINK_TAG, parseOrviloLink } from './parse';
 ...
 * custom `<orviloLink>` element so they can be rendered as rich inline chips.
 *
 ...
 * link renderer.
 */
export const rehypeOrviloLink = () => (tree: any) => {
  visit(tree, 'element', (node: any) => {
    if (node.tagName !== 'a') return;

    const href = node.properties?.href as string | undefined;
    const parsed = parseOrviloLink(href);
    if (!parsed) return;
