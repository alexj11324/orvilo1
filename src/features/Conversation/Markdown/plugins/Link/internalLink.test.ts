import { OFFICIAL_SITE, OFFICIAL_URL } from '@orvilo/const';
import { describe, expect, it } from 'vitest';

import { isBareLinkLabel, parseInternalLink } from './internalLink';

/**
 * The two official origins the parser recognizes. They are separate brand
 * constants on purpose — the app and the marketing site — even where a given
 * brand happens to serve both from the same host, so the cases below keep
 * naming the role they exercise rather than one host.
 */
const officialAppLink = (pathname: string) => new URL(pathname, OFFICIAL_URL).href;
const officialSiteLink = (pathname: string) => new URL(pathname, OFFICIAL_SITE).href;

describe('parseInternalLink', () => {
  it('parses official agent document links', () => {
    expect(
      parseInternalLink(officialAppLink('/agent/agt_agent/docs/document?mode=preview#title')),
    ).toEqual({
      agentId: 'agt_agent',
  ...
  it('parses global and agent-scoped tasks', () => {
    expect(parseInternalLink(officialAppLink('/task/T-198'))).toEqual({
      pathname: '/task/T-198',
      taskId: 'T-198',
      type: 'task',
    });
  ...
  it('ignores the readable title slug tail when resolving a task link', () => {
    expect(parseInternalLink(officialAppLink('/task/T-198/ship-the-thing'))).toEqual({
  ...
  it('parses verification report links', () => {
    expect(parseInternalLink(officialAppLink('/verify/run-1'))).toEqual({
  ...
  it('parses acceptance links as dedicated entities', () => {
    expect(parseInternalLink(officialAppLink('/acceptance/acceptance-1'))).toEqual({
    ...
    expect(parseInternalLink(officialSiteLink('/acceptance/acceptance-3'), OFFICIAL_URL)).toEqual({
  ...
  it('accepts the current self-hosted origin', () => {
    ...
    expect(
      parseInternalLink(officialAppLink('/task/T-200'), 'https://chat.example.com'),
    ).toBeNull();
  });

  it('accepts official links from the Electron renderer origin', () => {
    expect(parseInternalLink(officialAppLink('/task/T-201'), 'app://renderer')).toEqual({
      pathname: '/task/T-201',
      taskId: 'T-201',
      type: 'task',
    });
    expect(
      parseInternalLink(officialSiteLink('/acceptance/acceptance-4'), 'app://renderer'),
    ).toEqual({
  ...
  it('parses agent roots and keeps deeper routes as SPA routes', () => {
    ...
    expect(parseInternalLink(officialAppLink('/settings/profile'))).toEqual({
  ...
  it('leaves same-origin backend and framework links to the browser', () => {
    ...
    expect(parseInternalLink(officialAppLink('/f/generated-file-id'))).toBeNull();
...
describe('isBareLinkLabel', () => {
  it('treats a label that IS the address as bare', () => {
    ...
    expect(isBareLinkLabel(officialAppLink('/task/tsk_1'), officialAppLink('/task/tsk_1'))).toBe(
      true,
    );
  });
