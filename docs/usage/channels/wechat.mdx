---
title: Connect Orvilo to WeChat
description: >-
  Learn how to connect a WeChat bot to your Orvilo agent via the iLink Bot API,
...
# Connect Orvilo to WeChat

By connecting a WeChat channel to your Orvilo agent, users can interact with the AI assistant through WeChat private chats and group conversations.
...
In Orvilo, navigate to your agent's settings, then select the **Channels** tab. Click **WeChat** from the platform list.
...
<Steps>
  ...
  Open WeChat on your phone, go to **Scan** (via the + button in the top right), and scan the QR code displayed in Orvilo.
  ...
  Once confirmed, Orvilo will automatically save your credentials and connect the bot. You should see a success message in the channel settings.
</Steps>

## Step 3: Test the Bot

Open WeChat, find your bot contact, and send a message. The bot should respond through your Orvilo agent.
...
1. When you scan the QR code, Orvilo obtains a bot token from WeChat's iLink API
2. Orvilo continuously polls the iLink API for new messages (\~35 second intervals)
3. When a message arrives, it is routed through the Orvilo agent for processing
