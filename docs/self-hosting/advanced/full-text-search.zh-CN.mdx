---
title: 全文搜索
description: 为 Orvilo 产品搜索选择并运行 pg_search 或 Elasticsearch 后端。
tags:
  - 全文搜索
  - Elasticsearch
  - pg_search
...
Orvilo 使用全文搜索检索助手、话题、消息、文件、知识库、群聊和记忆等产品数据。这里的全文搜索与助手可调用的联网搜索工具无关。
...
`elasticsearch`，默认值是 `pg_search`。选中的后端不可用时，Orvilo 不会静默切换到另一个后端。

<Callout type="warning">
  设置 `FTS_SEARCH_PROVIDER=elasticsearch` 不会让历史数据库迁移跳过 `pg_search`。当前
  Elasticsearch 迁移流程面向已经执行过 `pg_search` 迁移的现有 Orvilo 数据库。对于无法安装
...
自建数据库时，Orvilo 的 Docker 示例使用 `paradedb/paradedb:latest-pg17` 镜像并预加载
`pg_search`。执行正常的 Orvilo 数据库迁移以安装扩展和项目维护的 BM25 索引，并保持：
...
2026 年 9 月 21 日从 Neon 移除，此后依赖它的查询、索引和应用都会停止工作。已经在 Neon 上运行 Orvilo 的用户应在截止日期前完成
...
`ES_INDEX_NAMESPACE`，区别只在于 Orvilo 如何连接集群。

#### 外部 Elasticsearch（Elastic Cloud 或托管集群）

- Orvilo 服务端可以通过 HTTPS 访问的 Elasticsearch 项目；
...
明文 HTTP 只允许用于本地开发时的回环地址。Orvilo 拒绝把 API 密钥通过明文 HTTP 发送到任何其他主机。

#### Docker Compose 单节点 Elasticsearch

官方 `docker-compose/deploy/docker-compose.yml` 附带一个默认关闭的可选 `elasticsearch`
服务，以及基于官方 Orvilo 镜像的回填和同步命令，适合希望在单机上使用 Elasticsearch、又不想申请外部账号的部署。该
...
选择 Elasticsearch 不会移除历史 `pg_search` 迁移。在这项后续工作完成前，所有 Orvilo 数据库（包括全新的 Docker
...
PostgreSQL 始终是数据事实来源。启用 Elasticsearch 路径后，采集触发器会把源数据变更合并写入持久化增量队列，持续运行的消费任务再把这些变更写入 Elasticsearch。搜索时，Elasticsearch 只返回候选标识，Orvilo 仍会回到 PostgreSQL 校验权限并读取最终结果。
...
| `fts-search-reindex` | `elasticsearch-reindex` | 基于 `alexj11324/orvilo1` 镜像的一次性回填 / 状态命令；checkpoint 保存在 `fts-search-reindex-state` 卷中 |
| `fts-search-sync`    | `elasticsearch-sync`    | 基于 `alexj11324/orvilo1` 镜像的长期增量消费任务；遇到失败或死信任务时退出并由 Compose 重启，同时输出日志                 |
...
路径保持不变：不设置这个变量时，Orvilo 仍然要求 HTTPS 和 API 密钥。
...
不同 Orvilo 部署不能共用同一个索引前缀。迁移或增量消费期间不能更改此前缀。

## 监控 Elasticsearch 搜索

Orvilo 会输出有限维度的 OpenTelemetry 指标和链路，不会记录原始查询、用户标识、文档标识或索引正文。主要指标包括：
