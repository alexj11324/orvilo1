...
**数据库连接失败** — 检查 `DATABASE_URL` 格式。对于托管数据库，确保末尾追加 `?sslmode=require`。如果 PostgreSQL 使用自签名证书，还要追加 `&uselibpqcompat=true`，让 Node 的 `pg` 客户端加密连接但不要求公共 CA。检查数据库 IP 白名单是否包含 Vercel IP（或使用 `0.0.0.0/0` 开放访问）。本地测试：`psql "$DATABASE_URL"`。
