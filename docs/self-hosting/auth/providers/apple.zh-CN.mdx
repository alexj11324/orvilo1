---
title: 在 Orvilo 中配置 Apple 身份验证
description: 学习如何在 Orvilo 中配置 Apple 登录，包括在 Apple Developer Portal 创建凭证和设置环境变量。
tags:
  - Apple
  - 身份验证
  - Orvilo
...
<Steps>
  ...
  5. 填写：
     - **Description**: 如 `Orvilo`
     - **Bundle ID**: 如 `com.yourcompany.orvilo`
  ...
  3. 填写：
     - **Description**: 如 `Orvilo Web`
     - **Identifier**: 如 `com.yourcompany.orvilo.web`（这是你的 Client ID）
...
<Callout type={'info'}>
  部署成功后，用户将可以通过 Apple 身份认证并使用 Orvilo。
</Callout>
