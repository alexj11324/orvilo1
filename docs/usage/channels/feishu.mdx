---
title: Connect Orvilo to Feishu (飞书)
description: >-
  Learn how to create a Feishu custom app and connect it to your Orvilo agent
...
# Connect Orvilo to Feishu (飞书)

By connecting a Feishu channel to your Orvilo agent, team members can interact with the AI assistant directly on Feishu through private chats and group conversations.
...
Orvilo supports two connection modes for Feishu bots:
...
For a new bot, use **创建飞书智能体应用** (Create Feishu Agent App) on the Feishu Open Platform. It presets permissions and events for an agent app. Connect it to your Orvilo agent by entering the resulting credentials in Orvilo. This creates the Feishu app; you still choose the agent it connects to in Orvilo.
...
<Steps>
  ...
  ![Create Feishu Agent App banner at the top of the developer portal](/images/docs/channels/feishu-agent-entry.png)

  ### Choose an Avatar and Name

  Select an avatar, enter an app name such as "Orvilo Assistant", and click **立即创建** (Create Now). Wait for the automatic configuration to finish.

  ![Choose an avatar and name for the Feishu agent app, then click Create Now](/images/docs/channels/feishu-agent-create.png)
  ...
  ![Created Successfully page showing App ID with App Secret hidden](/images/docs/channels/feishu-agent-created.png)

  ### Connect to Orvilo

  Open the agent you want to connect in Orvilo, then go to **Settings** → **Channels** → **飞书** (Feishu). Enter the **App ID** and **App Secret** you copied. Keep the default **WebSocket** connection mode under **Advanced Settings** and click **Save Configuration**.
  ...
  Follow the Feishu page prompts to confirm the app is published and available. Complete administrator approval first if your organization requires it. Click **打开应用** (Open App) and send the bot a message to confirm that your Orvilo agent replies.

  If it does not reply, check that the bot capability is enabled, the subscription method is **使用长连接接收事件** (Use long connection to receive events), `im.message.receive_v1` is included, and the channel configuration has been saved in Orvilo. See Troubleshooting below for further checks.
</Steps>

The screenshots show the Chinese Feishu interface with an example app named "Orvilo 周报小助手" (Orvilo Weekly Report Assistant). Use your own app name.
...
<Steps>
  ...
  Click **Create Enterprise App**. Fill in the app name (e.g., "Orvilo 助手"), description, and icon, then submit the form.
...
## Step 3: Configure Feishu in Orvilo

<Steps>
  ### Open Channel Settings

  In Orvilo, navigate to your agent's settings, then select the **Channels** tab. Click **飞书** (Feishu) from the platform list.
...
<Steps>
  ...
  Follow the path that matches the connection mode selected in Orvilo:

  - **WebSocket (recommended)** — Select **Use long connection to receive events** and save. Make sure the channel configuration has already been saved in Orvilo and the bot remains connected. No public Request URL is required.
  - **Webhook** — Select **Send events to developer server**, paste the **Event Subscription URL** copied from Orvilo into the **Request URL** field, and let the platform verify the endpoint.
  ...
  This allows your app to receive messages and forward them to Orvilo.
  ...
  Go back to Orvilo's channel settings and fill in:
...
Back in Orvilo's channel settings, click **Test Connection** to verify the credentials. Then find your bot on Feishu by searching its name and send it a message to confirm it responds.
...
To get it: DM the bot once and inspect the inbound event payload — the `open_id` field on the sender is yours. The Feishu Developer Portal also exposes a **User ID** lookup that maps mobile/email to `open_id`. Paste it into **Your Platform User ID** in Orvilo's Advanced Settings.
...
- **Bot not connecting (WebSocket mode):** Verify the App ID and App Secret are correct and that the Orvilo service can reach the Feishu Open Platform.
...
- **Event Subscription URL verification failed (Webhook mode):** Ensure you saved the configuration in Orvilo first, and the URL was copied correctly.
...
- **Bot ignores DMs:** Open **Advanced Settings** in Orvilo and check **DM Policy**. If it is `Disabled`, switch to `Open` or `Allowlist`. If it is `Allowlist`, confirm the sender's `open_id` is listed in **Allowed User IDs**.
