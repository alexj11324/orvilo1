...
/**
 * Shared dependencies every execution-lifecycle service operates on. Built
 * once per request scope by the compatibility facade so workspace/visitor
 * ownership (`userId`, `workspaceId`, share-visitor model access) stays
 * identical across the split services.
 */
 ...
 * `ChildRunService.completeSubAgentBridge`.
 */
 ...
 * `ChildRunService.completeGroupActionMember`. Mirrors the sub-agent bridge
...
export interface GroupActionMemberBridgeParams {
  ...
  /** Resume or finish the supervisor once all members complete (skipCallSupervisor/delegate). */
