...
description: 深入了解 Orvilo API 的前后端交互实现逻辑和核心组件。
tags:
  - Chat API
  - 前后端交互
  - Model Runtime
...
本文档说明了 Orvilo API 在前后端交互中的实现逻辑，
包括事件序列和涉及的核心组件。
...
- 使用 `@orvilo/fetch-sse` 包中的 `fetchSSE`
  发送请求到后端 API
...
Model Runtime（`packages/model-runtime/`）是 Orvilo 中
与 LLM 模型提供商交互的核心抽象层，
负责将不同提供商的 API 适配为统一接口。

**核心职责**：

- **统一抽象层**：通过 `OrviloRuntimeAI` 接口
