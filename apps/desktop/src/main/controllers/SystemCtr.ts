...
import type { ElectronAppState, ThemeMode } from '@orvilo/electron-client-ipc';
...
export default class SystemController extends ControllerModule {
  ...
  async getAppState(): Promise<ElectronAppState> {
    const { getShellInfo } = await import('@orvilo/local-file-shell/shell');
  ...
  async selectFolder(payload?: {
    ...
    const { detectRepoType } = await import('@orvilo/local-file-shell/git');
