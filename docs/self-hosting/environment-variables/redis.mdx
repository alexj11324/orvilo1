...
Orvilo uses Redis as a high-performance cache and session storage service. Configuring Redis can optimize authentication session management, file proxy caching, and more.
...
- Type: Optional
- Description: PEM-encoded CA certificate used to verify a private or self-hosted Redis TLS endpoint
- Default: System trust store
- Example: `-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----`

Literal `\n` sequences are converted to newlines, which allows the certificate to be stored in
single-line environment-variable providers. Keep certificate verification enabled for Redis
connections that cross a public network.
