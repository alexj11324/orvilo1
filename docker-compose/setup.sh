...
# ref: https://github.com/alexj11324/orvilo1/pull/5247
...
SOURCE_URL="https://raw.githubusercontent.com/alexj11324/orvilo1/main"
...
show_message() {
    ...
    case $key in
        ...
        tips_already_installed)
            case $LANGUAGE in
                zh_CN)
                    echo "检测到您已经运行过 Orvilo，本安装程序只能完成初始化配置，并不能重复安装。如果你需要重新安装，请删除 data 和 s3_data 文件夹。"
                ;;
                *)
                    echo "It is detected that you have run Orvilo. This installation program can only complete the initialization configuration and cannot be reinstalled. If you need to reinstall, please delete the data and s3_data folders."
                ;;
            esac
        ;;
        tips_run_command)
            case $LANGUAGE in
                zh_CN)
                    echo "您已经完成了所有配置。请运行以下命令启动 Orvilo 尝试启动："
                ;;
                *)
                    echo "You have completed all configurations. Please run this command to start Orvilo:"
                ;;
            esac
        ;;
        ...
        tips_if_run_normally)
            case $LANGUAGE in
                zh_CN)
                    echo "如果一切运行正常，你可以使用以下指令在 daemon 模式下启动 Orvilo:"
                ;;
                *)
                    echo "If everything runs normally, you can use the following command to start Orvilo in daemon mode:"
                ;;
            esac
        ;;
        tips_regen_jwks)
            case $LANGUAGE in
                zh_CN)
                    echo "在完成部署测试后，请前往 https://orvilo.aspectlylabs.com/docs/self-hosting/environment-variables/auth#jwks_key 生成新的 JWKS_KEY 并替换 .env 中的值，以确保安全性。"
                ;;
                *)
                    echo "After completing the deployment test, please go to https://orvilo.aspectlylabs.com/docs/self-hosting/environment-variables/auth#jwks_key to generate a new JWKS_KEY and replace the value in .env to ensure security."
                ;;
            esac
        ;;
        ...
        tips_show_documentation_url)
            case $LANGUAGE in
                zh_CN)
                    echo "https://orvilo.aspectlylabs.com/docs/self-hosting/environment-variables"
                ;;
                *)
                    echo "https://orvilo.aspectlylabs.com/docs/self-hosting/environment-variables"
                ;;
        ...
        ask_deploy_mode)
            case $LANGUAGE in
                zh_CN)
                    echo "请选择部署模式："
                    echo "(0) 域名模式（访问时无需指明端口），需要使用反向代理服务 Orvilo, RustFS，并分别分配一个域名；"
                    echo "(1) 端口模式（访问时需要指明端口，如使用IP访问，或域名+端口访问），需要放开指定端口；"
                    echo "(2) 本地模式（仅供本地测试使用）"
                    echo "如果你对这些内容疑惑，可以先选择使用本地模式进行部署，稍后根据文档指引再进行修改。"
                    echo "https://orvilo.aspectlylabs.com/docs/self-hosting/server-database/docker-compose"
                ;;
                *)
                    echo "Please select the deployment mode:"
                    echo "(0) Domain mode (no need to specify the port when accessing), you need to use the reverse proxy service Orvilo, RustFS, and assign a domain name respectively;"
