...
**为什么停在这里**：三项都要动 `apps/server` 与 `packages/database`。本轮禁止本机全仓类型检查与全仓测试（§0.4）—— 按包的定向验证**是**可行的（本轮已用 `packages/database` 的 PGlite 用例验证了项目产物过滤，`packages/builtin-tool-task` 也整包跑过），所以阻塞点不是「无法验证」这一般理由，而是：生成链走进程内 `createCaller`，其失败路径没有本地测试挂载点；第 1 项还要决定一张现有表的语义；第 2 项的恢复态需要一种新的结果形态。§5 已把「S30/S50 删除缺本地类型检查兜底」记为跟进项 1。
