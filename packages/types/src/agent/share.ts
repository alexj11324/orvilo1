 ...
 *
 * Lives in `@orvilo/types` (rather than next to the other agent-share
 * helpers in `@orvilo/const`) because both `@orvilo/const` and
 * `@orvilo/types`' own `AgentShareVisitorContext` need it, and `const`
