---
title: Configuring Authelia Authentication Service for Orvilo
description: >-
  Learn how to configure Authelia authentication service in Orvilo, including
  creating a provider, configuring environment variables, and deploying Orvilo.
...
<Steps>
  ...
  We assume you are already familiar with using Authelia. Let's say your Orvilo instance is deployed at [https://orvilo.example.com/](https://orvilo.example.com/). Note that currently only localhost supports HTTP access; other domains need to enable TLS, otherwise Authelia will actively interrupt authentication by default.

  Now, let's open and edit the configuration file of your Authelia instance:

  Add a new orvilo item under `identity_providers` -> `oidc`:
  ...
  Make sure to replace secret and `redirect_urls` with your own values. Note! The secret configured in Authelia is ciphertext, i.e., a salted hash value. Its corresponding plaintext needs to be filled in Orvilo later.
  ...
  | `AUTH_AUTHELIA_ID`        | Required | The id just configured in Authelia, example value is orvilo                                                                                                                                                                                               |
