---
title: Deploying Orvilo with Docker Compose
description: >-
  Learn how to deploy the Orvilo service using Docker Compose, including
...
# Deploying Orvilo Server Database Version with Docker Compose
...
Execute the following commands to set up the deployment environment; the directory `orvilo` will be used to store your configuration files and subsequent database files.

```sh
mkdir orvilo && cd orvilo
```

Fetch and execute the deployment script:

```sh
bash <(curl -fsSL https://raw.githubusercontent.com/alexj11324/orvilo1/main/docker-compose/setup.sh) -l en
```
...
<Steps>
  ...
  Visit your Orvilo service at [http://localhost:3210](http://localhost:3210).
</Steps>
...
<Steps>
  ...
  You can access your Orvilo service at `http://your_server_ip:3210`.
</Steps>
...
<Steps>
  ...
  - Domain setup for the Orvilo service: `orvilo.example.com`
  ...
  <Callout type="tip">
    **Single Domain Deployment**
    ...
    - Domain setup for the Orvilo service: `orvilo.example.com:${PORT1}`
    - Domain setup for the S3 service: `orvilo.example.com:${PORT2}`
  ...
  You can access your Orvilo service via `https://orvilo.example.com`.
</Steps>

## Custom Deployment

This section mainly introduces the configurations that need to be modified to customize the deployment of the Orvilo service in different network environments. Before starting, you can download the [Docker Compose configuration file](https://raw.githubusercontent.com/alexj11324/orvilo1/HEAD/docker-compose/deploy/docker-compose.yml) and the [environment variable configuration file](https://raw.githubusercontent.com/alexj11324/orvilo1/HEAD/docker-compose/deploy/.env.example).

```sh
curl -O https://raw.githubusercontent.com/alexj11324/orvilo1/HEAD/docker-compose/deploy/docker-compose.yml
curl -O https://raw.githubusercontent.com/alexj11324/orvilo1/HEAD/docker-compose/deploy/.env.example
mv .env.example .env
```

### Prerequisites

Generally, to fully run the Orvilo database version, you will need at least the following three services:

- The Orvilo database version itself
- PostgreSQL database with PGVector plugin
- Object storage service that supports S3 protocol

These services can be combined through self-hosting or online cloud services to meet various deployment needs. In this article, we provide a Docker Compose configuration entirely based on open-source self-hosted services, which can be used directly to start the Orvilo database version or modified to suit your requirements.
...
Orvilo needs to provide a public access URL for object files for the LLM service provider, so you need to configure the S3 Endpoint:
...
<Callout type="tip">
  For Docker Compose deployments, we recommend using `http://localhost:3210` (container calling itself) or `http://orvilo:3210` (using service name for container-to-container communication).
</Callout>
...
For production deployments with a custom domain and HTTPS, configure a reverse proxy in front of Orvilo.
...
server {
    ...
    server_name orvilo.example.com;
...
**Traefik** — add labels to the `orvilo` service in `docker-compose.yml`:

```yaml
labels:
  - "traefik.enable=true"
  - "traefik.http.routers.orvilo.rule=Host(`orvilo.example.com`)"
  - "traefik.http.routers.orvilo.entrypoints=websecure"
  - "traefik.http.routers.orvilo.tls.certresolver=letsencrypt"
```
...
docker compose logs -f orvilo

# Restart all services
docker compose restart

# Restart a specific service
docker compose restart orvilo
...
```sh
docker compose exec postgresql pg_dump -U postgres orvilo > backup.sql
```

Restore from backup:

```sh
docker compose exec -T postgresql psql -U postgres orvilo < backup.sql
```
...
docker cp orvilo-rustfs:/tmp/backup.tar.gz ./rustfs-backup.tar.gz
```
...
Common causes: port already in use (change `ORVILO_PORT` in `.env`), insufficient disk space, or database connection failure.
...
Verify the bucket was created: `docker compose exec rustfs ls /data/orvilo`
...
[docker-pulls-link]: https://hub.docker.com/r/alexj11324/orvilo1
[docker-pulls-shield]: https://img.shields.io/docker/pulls/alexj11324/orvilo1?color=45cc11&labelColor=black&style=flat-square
[docker-release-link]: https://hub.docker.com/r/alexj11324/orvilo1
[docker-release-shield]: https://img.shields.io/docker/v/alexj11324/orvilo1?color=369eff&label=docker&labelColor=black&logo=docker&logoColor=white&style=flat-square&sort=semver
[docker-size-link]: https://hub.docker.com/r/alexj11324/orvilo1
[docker-size-shield]: https://img.shields.io/docker/image-size/alexj11324/orvilo1?color=369eff&labelColor=black&style=flat-square&sort=semver
