...
function useSupportsWebGL2() {
  return useSyncExternalStore(subscribeToWebGL2Support, getWebGL2Support, getServerWebGL2Support);
}
...
function withColorAlpha(color: string, alpha: number) {
  ...
  if (!trimmed || !/^oklch\(/i.test(trimmed)) {
    return color;
  }

  return trimmed.replace(/\s*\/[^)]+(?=\))/, '').replace(/\)$/, ` / ${alpha})`);
}
...
export function OnboardingPageBackground() {
  ...
  const colorFront = withColorAlpha(
    getThemeColorToken('--muted-foreground', isDark ? 'oklch(0.708 0 0)' : 'oklch(0.556 0 0)'),
    isDark ? 0.24 : 0.22,
  );
