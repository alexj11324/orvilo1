---
title: Configuring Keycloak Authentication Service in Orvilo
description: >-
  Learn how to configure the Keycloak authentication service in Orvilo,
...
# Configuring Keycloak Authentication Service in Orvilo
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Orvilo via Docker
...
- Your Orvilo database version IP/port is `http://Orvilo_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Orvilo database version domain is `https://orvilo.example.com`.
- Your privately deployed Keycloak domain is `https://orvilo-auth-api.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Orvilo", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       - Valid redirect URIs:
         ...
         - Local network IP deployment: `http://Orvilo_IP:3210/api/auth/callback/keycloak`
         - Public environment: `https://orvilo.example.com/api/auth/callback/keycloak`
       - Web origins: Add your Orvilo domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Orvilo environment variables.

  Configure the Orvilo environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/Orvilo` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/Orvilo` for privately deployed Keycloak on a local network
  - `https://orvilo-auth-api.example.com/realms/Orvilo` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Orvilo. For Keycloak, fill in `keycloak`.                                                                                                           |
  ...
  | `AUTH_URL`                | Required | This URL specifies the callback address for Auth.js during OAuth verification. Only needed when the default generated redirect address is incorrect. `https://orvilo.example.com/api/auth` |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Orvilo.
</Callout>
