---
title: Orvilo 2.0 Breaking Changes
description: >-
  A comprehensive guide to breaking changes in Orvilo 2.0 and how to migrate
  from version 1.x.
tags:
  - Orvilo 2.0
  - Migration
...
# Orvilo 2.0 Breaking Changes

This document outlines the breaking changes introduced in Orvilo 2.0 and provides migration guidance for users upgrading from version 1.x.

## Removed Environment Variables

The following environment variables have been removed in Orvilo 2.0:
...
Orvilo 2.0 only supports Better Auth authentication system. The following environment variables are now required:
...
Orvilo 2.0 only supports Better Auth authentication system. NextAuth and Clerk are no longer supported.
...
Orvilo 2.0 only supports Server DB mode. Client DB (PGlite) is no longer supported. If you were using `NEXT_PUBLIC_SERVICE_MODE=client`, you need to migrate to Server DB deployment.

For deployment guides, see [Getting Started](/docs/self-hosting/start).

## PostgreSQL Version Requirements

Orvilo 2.0 recommends using **PostgreSQL 17** or higher.

Orvilo 2.0 introduced the [pg\_search](https://github.com/paradedb/paradedb/tree/main/pg_search)
