...
description: >-
  Explore the client-server interaction logic of Orvilo API, including
...
Orvilo API in client-server interactions,
...
- Uses `fetchSSE` from the `@orvilo/fetch-sse` package
...
- Frontend processes the streaming response
  `fetchSSE` and
...
Orvilo supports three types of tools:
...
**MCP Tools**: External tools connected
...
abstraction layer in Orvilo for interacting with
...
- **Unified Abstraction Layer**: Hides differences
  between AI provider APIs through the `OrviloRuntimeAI`
  interface
...
Agent Runtime (`packages/agent-runtime/`) is Orvilo's
