...
`@orvilo/agent-signal` is the shared Agent Signal domain package. It defines the language of Agent Signal nodes and source events, but it does not run policies, call model runtimes, read databases, trigger workflows, or import app server modules.
...
```ts
import type { AgentSignalSource, BaseAction, BaseSignal } from '@orvilo/agent-signal';
import { createAction, createSignal, createSource } from '@orvilo/agent-signal';
```
...
Import from `@orvilo/agent-signal/source` when producing, typing, validating, or routing source events:
...
```ts
import { AgentSignalScopeKey, getSourceEventScopeKey } from '@orvilo/agent-signal/source';
