...
export const SUMMARY_TAGS_PROMPT_VERSION = 'v1';

export const SUMMARY_TAGS_JSON_SCHEMA = {
  name: 'summary_tags',
  schema: {
    additionalProperties: false,
    properties: {
      tags: {
        description: 'Category tags in the target language, at most 5',
        items: { type: 'string' },
        type: 'array',
      },
    },
...
export const chainSummaryTags = (
  ...
  messages: [
    ...
    { content: '{"tags": ["起名", "写作", "创意"]}', role: 'assistant' },
    {
    ...
    { content: '{"tags": ["翻译", "写作", "文案"]}', role: 'assistant' },
    {
      content: `输入: {你是一名创业计划撰写专家，可以提供包括创意名称、简短的标语、目标用户画像、用户痛点、主要价值主张、销售/营销渠道、收入流、成本结构等计划生成。} [en-US]`,
      role: 'user',
    },
    { content: '{"tags": ["entrepreneurship", "planning", "consulting"]}', role: 'assistant' },
