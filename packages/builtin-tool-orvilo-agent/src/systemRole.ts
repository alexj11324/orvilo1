...
- The user wants a task to repeat automatically on a schedule (daily/weekly/hourly) — use **orvilo-task** and its scheduling capability instead. Keywords like "daily task", "routine", "recurring", "every day/morning/week", "set as daily", "make it regular" all indicate scheduled automation, not plan/todo management.
</when_to_use>
...
const baseSystemPrompt = `Use Orvilo Agent capabilities only when the active model needs built-in assistance. Prefer the active model's native capabilities whenever they are sufficient. Follow each tool's description and schema, and use tool results to answer the user directly.
