...
import { EXTERNAL_PUBLISH_ASSET_MAX_BYTES } from '@orvilo/device-control/file-preview';
import { getMimeType, resolveMimeType } from '@orvilo/utils/mimeType';
...
export class LocalFileProtocolManager {
  ...
  async readPreviewFile({
    ...
    // the device daemon's `defaultGetLocalFilePreview` (@orvilo/device-control).
    if (fileStat.size > MAX_DOCUMENT_PREVIEW_BYTES) {
