---
title: Use Orvilo on Telegram
description: >-
  Link your Orvilo account to the official Orvilo bot on Telegram. Send /start
...
# Use Orvilo on Telegram

Telegram is the simplest Messenger platform: there is one global Orvilo bot, and any Telegram account can DM it. You only need to link your Orvilo account once.
...
In Orvilo, open **Settings → Messenger** and click the **Telegram** card.
...
Either tap **Open in Telegram** in the modal, or scan the QR code with your phone. Telegram opens the official Orvilo bot.
...
Tap the link in the bot's reply. You'll be taken back to Orvilo. Sign in if asked, then choose a **default agent** — every message you DM the bot from now on will route to this agent.

{/* TODO: screenshot — confirm-link page in Orvilo with the agent picker visible */}
...
- **In Orvilo** — open **Settings → Messenger → Telegram** and use the agent picker on your link row.
...
> Disconnecting from Orvilo does not remove the bot from your Telegram chat list — you can manually delete the chat on Telegram if you no longer want to see it.
...
- **"This account is already linked"** — Your Telegram account is bound to a different Orvilo account. Sign in to that Orvilo account to manage the link, or unlink there first.
- **"Another Telegram account is already linked"** — Your Orvilo account already has a Telegram link. Disconnect it in **Settings → Messenger → Telegram** before linking a new Telegram account.
