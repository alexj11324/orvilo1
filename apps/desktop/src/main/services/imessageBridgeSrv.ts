...
} from '@orvilo/chat-adapter-imessage';
import type {
  ...
  private async forwardWebhook(
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logger.warn('Failed to read Orvilo webhook error response:', error);
      }
      throw new Error(detail || `Orvilo webhook failed with HTTP ${response.status}`);
    }
  }
}
