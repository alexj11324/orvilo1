...
title: >-
  ...
  Information
description: >-
  Learn how to configure the SearXNG online search functionality for Orvilo,
...
When enabled, Orvilo replaces the Jina endpoints as follows:
...
You can find publicly available SearXNG instances in the [SearXNG instance list](https://searx.space/). Choose an instance that is fast and reliable, and then configure its URL in Orvilo.

> Note that the `searxng` you use must have `json` output enabled; otherwise, the `orvilo` call will result in an error. If self-hosting, find the `searxng` configuration file and add `json` as shown below.
...
- Orvilo Online Search RFC Discussion
