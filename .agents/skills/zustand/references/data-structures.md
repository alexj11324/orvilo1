...
4. **Types from `@orvilo/types`** — never use `@orvilo/database` types in stores
...
3. **Don't use database types** — use types from `@orvilo/types`
...
Each entity gets its own file under `@orvilo/types/`. Each file exports two types:
...
import type { AgentEvalBenchmark, AgentEvalBenchmarkListItem } from '@orvilo/types';
