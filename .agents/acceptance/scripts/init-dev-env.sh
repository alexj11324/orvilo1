...
_hatchet_reachable() {
  local api_url="${HATCHET_CLIENT_API_URL:-}"
  [[ -n "$api_url" ]] || return 1
  command -v curl > /dev/null 2>&1 || return 2
...
_hatchet_grpc_reachable() {
  local host_port="${HATCHET_CLIENT_HOST_PORT:-}"
...
apply_env() {
  ...
  export HATCHET_CLIENT_API_URL HATCHET_CLIENT_HOST_PORT HATCHET_CLIENT_NAMESPACE
...
cmd_preflight() {
  ...
  if [[ "$AGENT_RUNTIME_MODE" == "queue" ]]; then
    if _hatchet_configured && _hatchet_reachable && _hatchet_grpc_reachable; then
      ...
      note "set HATCHET_CLIENT_TOKEN, HATCHET_CLIENT_API_URL, and optional HATCHET_CLIENT_HOST_PORT"
