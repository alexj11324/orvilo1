...
**Database connection fails** — Verify `DATABASE_URL` format. Ensure `?sslmode=require` is appended for managed databases. If the PostgreSQL server uses a self-signed certificate, also append `&uselibpqcompat=true` so Node's `pg` client encrypts the connection without requiring a public CA. Check that the database's IP allowlist includes Vercel IPs (or use `0.0.0.0/0` for open access). Test locally with `psql "$DATABASE_URL"`.
