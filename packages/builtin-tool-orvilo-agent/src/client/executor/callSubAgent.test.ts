import type { BuiltinToolContext } from '@orvilo/types';
import { describe, expect, it, vi } from 'vitest';

import { orviloAgentExecutor } from './index';
...
describe('orviloAgentExecutor.callSubAgent', () => {
  ...
  it('persists the full spend the runner reports, not just the token total', async () => {
    ...
    const result = await orviloAgentExecutor.callSubAgent(params, createContext(run));
  ...
  it('fails when the runtime provides no sub-agent runner', async () => {
    const result = await orviloAgentExecutor.callSubAgent(
      params,
