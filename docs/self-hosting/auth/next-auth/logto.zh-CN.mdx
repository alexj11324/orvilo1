---
title: 在 Orvilo 中配置 Logto 身份验证服务
description: 学习如何在 Orvilo 中配置 Logto 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Logto 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'warning'}>
  NextAuth（Auth.js）已从 Orvilo 中完全移除。下文出现的 `NEXT_AUTH_SSO_PROVIDERS` 和 `AUTH_URL` 等变量现在会导致部署在构建 / 启动阶段失败。本页仅作历史参考保留，请改用 [Better Auth 提供商指南](/zh/docs/self-hosting/auth) 和 [NextAuth 迁移至 Better Auth 指南](/zh/docs/self-hosting/migration/v2/auth/nextauth-to-betterauth)。
</Callout>

[Logto](https://github.com/logto-io/logto) 是一个开源的身份验证服务，界面简洁美观、功能配置丰富且易于上手，你即可以选择使用其官方提供的 Logto Cloud，也可以选择私有部署 Logto。

<Callout type={'tip'}>
  若你想要私有部署 Logto，我们建议你将之与 Orvilo 一同使用 Docker Compose 部署，此时
  Orvilo 可以与之共用同一个 Postgres 实例。
</Callout>

## Logto 配置流程

下文假设你的 Orvilo 域名为 `https://orvilo.example.com`。

若你是私有部署的 Logto，假设其 endpoint 域名为 `https://orvilo-auth-api.example.com`。
...
<Steps>
  ...
  配置 `Redirect URI` 为 `https://orvilo.example.com/api/auth/callback/logto`，`Post sign-out redirect URI` 为 `https://orvilo.example.com/`

  配置 `CORS allowed origins` 为 `https://orvilo.example.com`
  ...
  配置 Logto 的 Webhook，以便在用户信息更新时 Orvilo 可以接收到通知。

  前往 `Webhooks` ，创建一个 Webhook，填写以下字段：

  - 端点 URL： `https://orvilo.example.com/api/webhooks/logto`
  - 事件：
    - `User.Data.Updated`: 允许 Orvilo 同步 Logto 中用户资料信息的更新。
    - `User.SuspensionStatus.Updated`: 允许 Orvilo 将被暂停的用户移除登录会话，仅在数据库会话策略为 `database` 时可用。
  ...
  将获取到的 `Client ID` 和 `Client Secret`，设为 Orvilo 环境变量中的 `AUTH_LOGTO_ID` 和 `AUTH_LOGTO_SECRET`。

  配置 Orvilo 环境变量中 `AUTH_LOGTO_ISSUER` 为：

  - `https://orvilo-auth-api.example.com/oidc`，若你是私有部署的 Logto
  - `https://example.logto.app/oidc`，若你是使用的 Logto Cloud

  在部署 Orvilo 时，你需要配置以下环境变量：
  ...
  | `AUTH_URL`                  | 必选 | 该 URL 用于指定 Auth.js 在执行 OAuth 验证时的回调地址，当默认生成的重定向地址发生不正确时才需要设置。`https://orvilo.example.com/api/auth` |
...
<Callout type={'info'}>部署成功后，用户将可以通过 Logto 身份认证并使用 Orvilo。</Callout>
