...
import {
  ...
  resolveAgainstCwd,
  writeLocalFile,
} from '@orvilo/local-file-shell/file';
import type { FileResult, SearchOptions } from '@orvilo/local-file-shell/types';
import { resolveMimeType } from '@orvilo/utils/mimeType';
...
export default class LocalFileCtr extends ControllerModule {
   ...
   * `@orvilo/device-control`) on ONE cache directory and one proxy-aware
  ...
  async handlePrepareSkillDirectory(
    ...
    const { prepareSkillDirectory } = await import('@orvilo/device-control/skill-directory');
