---
title: Orvilo Model Service Providers - Environment Variables and Configuration
description: >-
...
When deploying Orvilo, a rich set of environment variables related to model service providers is provided, allowing you to easily define the model service providers to be enabled in Orvilo.
...
- Why is the return value blank after installing Docker, configuring environment variables?
- Reasons for errors when using third-party interfaces
- No response in chat after filling in the proxy server address
...
You can find all current model names in [modelProviders](https://github.com/alexj11324/orvilo1/tree/main/packages/model-bank/src/modelProviders).

## Meta

Use the official [Meta Model API](https://dev.meta.ai/).

### `META_API_KEY`

- Type: Optional for the deployment; required when providing a shared server key.
- Description: A key created in the [Model API dashboard](https://dev.meta.ai/). Setting a non-empty key automatically enables Meta in the server configuration. `ENABLED_META` is derived from this key, not a separate environment switch. Orvilo uses `META_API_KEY`, rather than the `MODEL_API_KEY` name in Meta's SDK examples.
...
- Type: Required
- Description: A JSON string of your Google Cloud service account key; see the [Vertex AI documentation](https://cloud.google.com/vertex-ai/docs/start/authentication) for how to obtain it.
- Default: -
...
- Type: Optional
- Description: Used to control the model list, use `+` to add a model, use `-` to hide a model, use `model_name=display_name` to customize the display name of a model, separated by commas. Definition syntax rules
- Default: `-`
...
Connect to your own Unsloth Studio instance.
...
- Type: Optional
- Description: Overrides the OpenAI-compatible API base URL. Use an address reachable from the Orvilo server. Inside Docker, `127.0.0.1` refers to the container, not the host running Studio. An explicitly configured Studio port overrides its default `8888`.
