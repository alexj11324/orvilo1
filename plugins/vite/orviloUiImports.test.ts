...
import { orviloUiImports, parseBarrel } from './orviloUiImports';

const ENTRY_ID = '\0orvilo-ui-imports-fixture.mjs';

const fixturePlugin: Plugin = {
  name: 'orvilo-ui-imports-fixture',
  load(id) {
  ...
  resolveId(id) {
    if (id === 'virtual:orvilo-ui-imports-fixture') return ENTRY_ID;
  },
};
    ...
    const result = await build({
      build: {
        ...
        rolldownOptions: {
          ...
          input: 'virtual:orvilo-ui-imports-fixture',
        },
        write: false,
      },
      configFile: false,
      logLevel: 'silent',
      plugins: [fixturePlugin, ...orviloUiImports()],
    });
