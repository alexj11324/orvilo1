...
<Steps>
  ...
  点击左侧边栏的 `对象存储` 菜单，右上角 `创建存储桶` 按钮，创建一个新的存储桶（Bucket）。创建存储桶时将指定其名称，下文以 `orvilo` 为例。版本、对象锁依照默认配置不开启。
  ...
  | 域名                          | 反代端口   | 是否必选 |
  | --------------------------- | ------ | ---- |
  | `orvilo-s3-api.example.com` | `9000` | 必选   |
  ...
  修改 Orvilo 的 `.env` 文件，添加如下环境变量，即可完成配置，使用 RustFS 作为 S3 存储服务：
