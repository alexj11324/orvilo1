import { builtinSkills, OrviloIdentifier } from '@orvilo/builtin-skills';
import { renderPlaceholderTemplate } from '@orvilo/context-engine';
import { describe, expect, it } from 'vitest';

const OrviloSkill = builtinSkills.find((s) => s.identifier === OrviloIdentifier);
if (!OrviloSkill) {
  throw new Error(`OrviloSkill not found in builtinSkills (looking for "${OrviloIdentifier}")`);
}
const orviloHubContent = OrviloSkill.content;
 ...
 * the Orvilo builtin skill content references `{{agent_id}}`, `{{topic_id}}`,
 * etc., and `contextEngineering.ts` provides the matching variable generators.
 *
 * This test pins the contract from BOTH ends:
 *   1. `aspectlylabs/content.ts` actually contains the expected `{{...}}` tokens.
...
describe('Orvilo skill identity placeholders ()', () => {
  ...
  it('orviloHubContent references all expected placeholder tokens', () => {
    const content = orviloHubContent;
  ...
  it('renderPlaceholderTemplate substitutes all identity placeholders', () => {
    const rendered = renderPlaceholderTemplate(orviloHubContent, {
      agent_id: 'agt_xyz',
