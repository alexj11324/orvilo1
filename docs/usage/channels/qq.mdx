---
title: Connect Orvilo to QQ
description: >-
  Learn how to create a QQ bot and connect it to your Orvilo agent as a message
...
# Connect Orvilo to QQ

By connecting a QQ channel to your Orvilo agent, users can interact with the AI assistant through QQ group chats, guild channels, and direct messages.
...
Orvilo supports two connection modes for QQ bots:
...
<Steps>
  ...
  If you are using **Webhook mode**, navigate to **Development Settings** → **Callback Configuration** in the QQ Open Platform. You will need to paste the Orvilo Callback URL here after completing Step 2.

  If you are using **WebSocket mode** (default), skip this step — no callback URL is needed.
</Steps>

## Step 2: Configure QQ in Orvilo

<Steps>
  ### Open Channel Settings

  In Orvilo, navigate to your agent's settings, then select the **Channels** tab. Click **QQ** from the platform list.
...
<Steps>
  ...
  Go back to the QQ Open Platform, navigate to **Development Settings** → **Callback Configuration**. Paste the **Callback URL** you copied from Orvilo.
  ...
  The QQ Open Platform will send a verification request to your Callback URL. Orvilo handles this automatically using Ed25519 signature verification.
</Steps>
...
Click **Test Connection** in Orvilo's channel settings to verify the integration. Then open QQ, find your bot, and send a message. The bot should respond through your Orvilo agent.
...
To get it: DM the bot once with any message and check the server logs for the `user_openid` field on the inbound event payload (or read it from the OpenAPI dashboard if available). Paste that ID into **Your Platform User ID** in Orvilo's Advanced Settings.
...
- **Callback URL verification failed (Webhook mode):** Ensure you saved the configuration in Orvilo first and the URL was copied correctly. Orvilo handles Ed25519 verification automatically.
...
- **Test Connection failed:** Double-check the App ID and App Secret in Orvilo's channel settings.
