import type { SaveUserQuestionField } from '@orvilo/types';
...
export const formatWebOnboardingStateMessage = (state: OnboardingStateContext) => {
  ...
  const parts: string[] = [
    phaseGuidance,
    'Questioning rule: prefer the `orvilo-user-interaction____askUserQuestion` tool call for structured collection or explicit UI input. For natural exploratory questions, plain text is allowed.',
  ];
