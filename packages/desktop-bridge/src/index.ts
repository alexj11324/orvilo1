 ...
 * interceptor cannot tell an internal Orvilo URL from a third-party one by origin
 ...
 */
export const RENDERER_HANDLED_LINK_ATTR = 'data-orvilo-renderer-link';
 ...
 * originates from the Market service's own OAuth token, NOT from the user's Orvilo
 * session.  responseMeta checks this to suppress the X-Auth-Required header so the
 * desktop "re-login to Orvilo" modal is NOT shown; the Market OAuth flow handles it
