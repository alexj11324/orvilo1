...
2. **关联失败的可恢复态含一处真缺陷**：`agentDocuments.ts` 里 `pinToTask` 在 `withDocumentOutcome` **之外**被调用，而后者在 operation 成功时已先发出 `status: 'succeeded'`。于是 pin 抛错时：产物 outcome 已记成功 → 随后整个工具调用抛给 agent。库里留下一份文件、没有任务关联、信号自相矛盾。修法不是简单 catch（那会变成静默 —— 审计已经指出 `pinDocument` 的 `.onConflictDoNothing()` 与 Work 注册的 `catch { log }` 都在静默失败），而要按 §8 呈现「文件已生成但关联失败」并给出**只重挂、不重生成**的恢复动作。这是服务端结果形态 + 客户端渲染 + agent 可见语义的跨层改动。
3. **删除底层文件的引用保护**：`task_documents.documentId → documents.id` 是 cascade，从资源库删文档会无声摘掉所有任务上的产物引用，全仓没有任何引用计数或二次确认。

**为什么停在这里**：三项都要动 `apps/server` 与 `packages/database`。本轮禁止本机类型检查与全仓测试（§0.4），生成链是进程内 `createCaller`，改错只会在 `apps/server` 的 tsc 暴露。第 1 项还会决定一张现有表的语义，第 2 项的恢复态需要一种新的结果形态。§5 已把「S30/S50 删除缺本地类型检查兜底」记为跟进项 1 —— 这是同一条约束的又一次体现。
