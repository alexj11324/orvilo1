...
LobeHub 采用 Monorepo 架构（`@orvilo/` 命名空间），
顶层目录结构如下：
...
- **`src/spa/`** — SPA 入口（`entry.mobile.tsx`、`entry.desktop.tsx`、
  `entry.popup.tsx`、`entry.auth.tsx`）和 `router/` 下的 React Router 配置。
  没有浏览器入口：产品只发布移动端 SPA 和 Electron 桌面端。
...
└── router/
    ├── desktopRouter.shared.tsx           # 桌面端共享路由定义
    ├── desktopRouter.config.tsx           # Electron 适配器（没有 .desktop 变体：
    │                                      # tsgo 和 vitest 不走 platformResolve）
...
渲染移动端 HTML 外壳，随后加载 `src/spa/entry.mobile.tsx` 来挂载 React Router 应用。
Electron 桌面端自带外壳，加载的是 `entry.desktop.tsx`。

**关键配置文件：**

- 桌面端路由：`src/spa/router/desktopRouter.config.tsx`（基于 `desktopRouter.shared.tsx` 的 Electron 适配器）
