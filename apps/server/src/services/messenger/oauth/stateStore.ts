...
const log = debug('orvilo-server:messenger:oauth-state');
...
export interface OAuthStatePayload {
  /** Orvilo user who clicked "Connect Slack". Persisted on the install row. */
  orviloUserId: string;
...
/**
 * Issue a single-use OAuth state token bound to the Orvilo user who
...
export const issueOAuthState = async (payload: Omit<OAuthStatePayload, 'ts'>): Promise<string> => {
  ...
  log('issueOAuthState: issued state for user=%s ttl=%ds', payload.orviloUserId, STATE_TTL_SECONDS);
  return state;
};
