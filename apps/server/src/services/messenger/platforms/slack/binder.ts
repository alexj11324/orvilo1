...
const log = debug('orvilo-server:messenger:slack');
...
export class MessengerSlackBinder implements MessengerPlatformBinder {
  ...
  async handleUnlinkedMessage(ctx: UnlinkedMessageContext): Promise<void> {
    ...
    try {
      ...
      const errorText = 'Orvilo is temporarily unavailable. Please try again in a moment.';
    ...
    if (ctx.channelMentionThreadId) {
      const [, channelId, threadTs] = ctx.channelMentionThreadId.split(':');
      const text =
        "Hi, I'm Orvilo — your AI agent on Slack.\n" +
        `Link your Orvilo account to start chatting: <${verifyUrl}|click here>`;
  ...
  async notifyLinkSuccess(params: {
    ...
    const headline =
      ':white_check_mark: Linked successfully! Your Orvilo account is now connected.';
   ...
   * `@Orvilo` in a public channel — we want them to get the verify-im URL
