...
#
# This script initializes the env needed to run Orvilo's normal local dev
...
#
# Overrides:
#   SERVER_PORT=3010 DB_PORT=5433 DB_CONTAINER=orvilo-agent-testing-postgres REDIS_PORT=6380 REDIS_CONTAINER=orvilo-agent-testing-redis S3_DEV_PORT=29000
...
# (.../orvilo-cloud*/orvilo), REPO_ROOT is the submodule but the shared
...
if [[ "$(basename "$WORKSPACE_ROOT")" == "orvilo" ]]; then
  _parent_root="$(cd "$WORKSPACE_ROOT/.." && pwd)"
  [[ "$(basename "$_parent_root")" == orvilo-cloud* ]] && WORKSPACE_ROOT="$_parent_root"
fi

# --- auto-allocated, non-conflicting ports (persisted per workspace) ---------
# Each repo copy (orvilo-cloud, orvilo-cloud-cc, ...) probes its own free
...
DB_CONTAINER="${DB_CONTAINER:-orvilo-agent-testing-postgres}"
DATABASE_URL="${DATABASE_URL:-postgresql://postgres:postgres@localhost:${DB_PORT}/postgres}"
REDIS_PORT="${REDIS_PORT:-6380}"
REDIS_CONTAINER="${REDIS_CONTAINER:-orvilo-agent-testing-redis}"
...
AGENT_TESTING_API_KEY="${AGENT_TESTING_API_KEY:-sk-ov-agenttesting0001}"
...
write_env() {
  ...
  {
    printf '# Source this file before starting Orvilo local dev server.\n'
...
const TEST_API_KEY = {
  ...
  key: process.env.AGENT_TESTING_API_KEY || 'sk-ov-agenttesting0001',
  name: 'Agent Testing CLI API Key',
};

const validateApiKeyFormat = (apiKey) => /^sk-ov-[\da-z]{16}$/.test(apiKey);
...
const writeCliEnvFile = () => {
  ...
  fs.writeFileSync(
    file,
    [
      '# Source this file before running Orvilo CLI agent tests.',
      '# Generated by init-dev-env.sh seed-user',
      `export ORVILO_API_KEY=${TEST_API_KEY.key}`,
      `export ORVILO_CLI_API_KEY="${'${ORVILO_API_KEY}'}"`,
      `export ORVILO_SERVER=${process.env.APP_URL}`,
      'export ORVILO_CLI_HOME=.orvilo-dev',
      '',
    ].join('\n'),
  );
...
(async () => {
  ...
  console.log(`  ORVILO_API_KEY: ${TEST_API_KEY.key}`);
