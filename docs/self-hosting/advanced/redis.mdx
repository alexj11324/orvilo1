...
description: >-
  Learn how to configure Redis cache service to optimize Orvilo performance and
  session management.
tags:
  - Redis
  - Cache
...
Orvilo uses Redis as a high-performance cache and session storage service to optimize system performance and manage user authentication state.

<Callout type={'info'}>
  Orvilo uses the standard Redis protocol (via ioredis library), supporting any Redis
...
Redis is used in Orvilo for the following scenarios:
...
<Steps>
  ...
  The prefix for Redis keys, used to isolate Orvilo data in a shared Redis instance.

  - Default: `orvilo`
  - Example: `REDIS_PREFIX=my-orvilo`
...
<Callout type={'warning'}>
  Redis is an optional service. If `REDIS_URL` is not configured, Orvilo will still function
