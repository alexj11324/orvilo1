...
LobeHub uses a Monorepo architecture (`@orvilo/` namespace).
...
│   ├── entry.mobile.tsx / entry.desktop.tsx / entry.popup.tsx / entry.auth.tsx
...
- **`src/spa/`** — SPA entry points (`entry.mobile.tsx`, `entry.desktop.tsx`,
  `entry.popup.tsx`, `entry.auth.tsx`) and React Router config under `router/`. There is no
  browser entry: the product ships the mobile SPA and the Electron desktop app.
...
└── router/
    ├── desktopRouter.shared.tsx           # Shared desktop route definition
    ├── desktopRouter.config.tsx           # Electron adapter (no .desktop variant: tsgo
    │                                      # and vitest do not run platformResolve)
...
renders the mobile HTML shell, which then boots `src/spa/entry.mobile.tsx` to mount the
React Router app. The Electron app serves its own shell and boots `entry.desktop.tsx`.

**Key configuration files:**

- Desktop router: `src/spa/router/desktopRouter.config.tsx` (Electron adapter over `desktopRouter.shared.tsx`)
