...
import { BRANDING_NAME, SOCIAL_URL } from '@orvilo/business-const';
...
describe('Metadata', () => {
  ...
  describe('genTwitter', () => {
    it('should generate Twitter metadata correctly', () => {
      ...
      // `site` names the X account this deployment owns, so it tracks
      // SOCIAL_URL.x — a deployment without an X account advertises none,
      // rather than falling back to a handle it does not control. Asserted
      // separately because toEqual ignores `undefined` properties, which would
      // let an unexpected handle slip through in the unset case.
      if (SOCIAL_URL.x) {
        expect(result.site).toMatch(/^@\S+$/);
      } else {
        expect(result.site).toBeUndefined();
      }
    });

    it('never advertises the upstream X handle', () => {
      const result = (meta as any).genTwitter({
        title: 'Twitter Title',
        description: 'Twitter description',
        image: 'https://twitter-image.com',
