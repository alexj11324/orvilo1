...
Orvilo 支持使用 Better Auth 配置外部身份验证服务，供企业 / 组织内部使用，统一管理用户授权。
...
要在 Orvilo 中启用 Better Auth，请设置以下环境变量：

| 环境变量                   | 类型  | 描述                                                                                                                                                                                  |
| ---------------------- | --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_SECRET`          | 必选  | 用于加密会话令牌的密钥。使用以下命令生成：`openssl rand -base64 32`                                                                                                                                      |
| `APP_URL`              | 必选  | 本实例的公网访问地址（例如 `https://orvilo.example.com`）。Better Auth 将其作为 `baseURL` 生成 OAuth 回调与 trusted origins。Vercel 上未设置时可回退到 `VERCEL_*`；其他环境若省略会默认 `http://localhost:<PORT>`，导致公网域名 SSO 失败。 |
...
| `REDIS_PREFIX` | 可选 | Redis 键前缀，默认为 `orvilo`          |
