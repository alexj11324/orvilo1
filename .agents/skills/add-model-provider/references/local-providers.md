...
During the September 2026 integration, `/v1/models` advertised a loaded GGUF model and its configured context length but omitted tool and vision flags. Studio's `/props` exposed `model_path`, `chat_template_caps`, and `modalities`; matching that ID allowed capability discovery without guessing from the model name. The Orvilo adapter at that revision records the implementation. Inspect the current [Unsloth compatibility route](https://github.com/unslothai/unsloth/blob/main/studio/backend/routes/llama_compat.py) before reusing it: these fields and access policies are provider- and version-specific.
