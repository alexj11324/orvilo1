...
import type { ExecutionSnapshot, ISnapshotStore, StepSnapshot } from '@orvilo/agent-tracing';
import { messages } from '@orvilo/database/schemas';
import { context as otContext, SpanStatusCode } from '@orvilo/observability-otel/api';
import {
  tracer,
  workflowRunCounter,
  workflowRunDurationHistogram,
} from '@orvilo/observability-otel/modules/agent-signal';
import { attributesCommon } from '@orvilo/observability-otel/node';
...
const createDefaultSnapshotStore = (): ISnapshotStore | null => {
  ...
  try {
    ...
    const { FileSnapshotStore } = require('@orvilo/agent-tracing');
