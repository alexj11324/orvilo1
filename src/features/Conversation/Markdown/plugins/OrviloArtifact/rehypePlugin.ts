    ...
    visit(tree, (node, index, parent) => {
      if (node.type === 'element' && node.tagName === 'p' && node.children.length > 0) {
        ...
        if (firstChild.type === 'raw' && firstChild.value.startsWith(`<${ARTIFACT_TAG}`)) {
          // Extract orviloArtifact attributes
          ...
          // Create new orviloArtifact node
          const newNode = {
            children: [
              {
                type: 'text',
      ...
      // If the string is <orviloArtifact identifier="ai-new-interpretation" type="image/svg+xml" title="New AI Interpretation">
      // The resulting node is:
      // {
      //   type: 'raw',
      //   value:
      //     '<orviloArtifact identifier="ai-new-interpretation" type="image/svg+xml" title="New AI Interpretation">',
      // }
