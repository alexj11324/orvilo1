---
title: Deploying Orvilo Database with Docker
description: >-
  Learn how to deploy the Orvilo server database version using Docker on Linux
  and local machines.
tags:
  - Orvilo
  - Docker
...
Here is the process for deploying the Orvilo server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    components for Orvilo to implement RAG and knowledge base search.

    It should be noted that the new version of ParadeDB requires explicitly enabling the pg\_search extension in order to be used by Orvilo. Here we explicitly enable pg\_search.
  </Callout>
  ...
  ### Create a file named `orvilo.env` to store environment variables:
  ...
  ### Start the orvilo Docker image

  ```sh
  docker run -it -d -p 3210:3210 --network pg --env-file orvilo.env --name orvilo alexj11324/orvilo1
  ```
...
The data version of Orvilo also supports direct use on a local Mac/Windows machine.
...
```shell
$ docker run -it -d --name orvilo -p 3210:3210 \
