...
| S20 默认看板、旧首页卸载 | IMPLEMENTED | CI\_PENDING                        | `fb1a52a6` `e2011272` `a538be19` `f1cd2d5a` `735c7972` `bebd90a8` `2f6c5712` `919ef953` | **已完成**：默认看板、Web 落地任务列表、**Web 侧卸载 Home**、**收件箱薄路由 `/inbox`**（§2.2）、**首页画像移除**（`HomePortrait`/`PortraitBubble`/`GreetingLine` 与 `showHomePortrait` 偏好，§1.7.2）、**落点核对与决定**（§2.2.2：深链接优先满足 NAV-07；Electron 改由 `DEFAULT_HOME_MODE = 'task'` 落地）、**`/inbox` 保留段修复 + 同步守卫**（`bebd90a8`）、**重复宿主两处修复**（`TopicChatDrawer` 全局宿主 context；`AcceptancePortalDrawer` 的 `portalColumn` 路由元数据 + 「列命中且面板展开」复合条件，一个 handle 覆盖 `/tasks` `/inbox` `/task/*` `/goal/*`）。**未完成的部分与其它 stage 同类，均为 S80 / 真机专属**：①「真实界面只出现一层」未在真机确认；②折叠几何与 Electron 开屏观感未在渲染页验证；③基线自带红点 `src/services/chat/chat.test.ts`（在 `main` 上即红，已对照确认非本轮引入，见 §6.3）。 |     |     |
...
| S80 远端验收与证据       | IMPLEMENTED | CI\_GREEN (1 out-of-scope failure) | `bb7ccebe`                                                                              | CI 已跑完并取证（§7）：Test CI **12/12 job 全绿**（含 `Typecheck`）；E2E 83 场景中 2 条失败，均为文稿 `PAGE-*`（§7.6.4：非本轮、main 已删该套件、失败集合在两次运行间漂移）。第一次运行暴露的 4 类失败已逐条定性并修复（§7.6.1–§7.6.3），其中 2 条 Home E2E 场景按 S20 退役并留档。**产品级验收 BLOCKED**（§7.4）：§13.1 要求验收在 GHA / 已授权测试环境完成，本机不启动重环境；Electron 与移动端真实渲染面、原生菜单与标签恢复均无自动化覆盖。                                                                                                                                                                                                                                                                                                                                                                        |     |     |
...
- **`AcceptancePortalDrawer` 会与常驻 portal 栏重复渲染同一份验收内容 —— 已修复（2026-09-16）。**
  抽屉的开关条件是 `isAcceptancePortalView(viewType)`，原本**没有任何路由守卫**，`GlobalOverlays/index.tsx:48` 用同一条件 arm。

  **触发它的正是产品自己的流程**：`AgentTasks/AgentTaskDetail/useOpenAcceptanceInPanel.ts:17-24`
  把验收视图推入 portal 栈**并且** `showTaskAgentPanel(true)` 展开面板
  （`TaskAcceptance.tsx:161,168`、`RunVerifyTag.tsx:93`、`DailyBrief/BriefActionLink.tsx:77` 同样）。
  于是 `AgentTaskManager/index.tsx:24-38` 的 `showAcceptance` 与 `expand` 同时为真，它的 `<RightPanel>`
  渲染 `<PortalContent />`，而抽屉读同一个 `currentViewType` 再渲染一份。
  根因值得记：`AgentTaskManager:25-26` 是把 `isAcceptancePortalView` 的谓词**手抄**了一遍 ——
  同一个谓词两份拷贝，于是它选中的内容也有了两份。

  **关键结构（它决定了修法能有多简单）**：`desktopRouter.shared.tsx` 里**一个无 path 的包装路由**，
  children 是 `tasks`(`:899`)、`inbox`(`:915`)、`task`(`:930`)、`goal`(`:944`)，
  element 是 `TaskWorkspaceLayout`(`:947-950`) → `{isMobile ? <MobilePortal /> : <AgentTaskManager />}`。
  即 **`/tasks`、`/inbox`、`/task/*`、`/goal/*` 全都在常驻列之下** —— 所以**一个 handle 覆盖四条**，
  不必枚举路径，工作区镜像 `/:slug/*` 也自动覆盖。

  **实现**：① `RouteMeta` 加 `portalColumn?: true`（`src/spa/router/routeMeta.ts`）；
  ② 那个包装路由声明 `handle: { meta: routeMeta({ portalColumn: true }) }`；
  ③ 新 hook `GlobalOverlays/usePortalColumnHost.ts`，用 `useMatches()` + 既有的 `getRouteMetaFromHandle` 读；
  ④ 抽屉条件改为 `isAcceptancePortalView(viewType) && !(columnHost && columnExpanded)`。

  **为什么必须是复合条件 —— 这是本改动的安全性来源**：`@lobehub/ui` 的 `DraggablePanel`
  **无论展开与否都挂载 children**，只在折叠时把尺寸置 0（`DraggablePanel.mjs` 的 `:362/:429` 挂 children，
  `:371/:379` 是 `isExpand ? size : 0`）。所以折叠的列把验收视图**藏着但不显示**。
  只按「命中列路由」退让，会让用户在面板折叠时**什么都看不到**。加上「且面板已展开」后三种情形都正确：

  | 情形                   | 列  | 面板展开 | 抽屉 | 结果                   |
  | ---------------------- | --- | -------- | ---- | ---------------------- |
  | 无列路由（如 `/page`） | —   | —        | 开   | 唯一宿主，内容可见     |
  | 列路由 + 面板展开      | ✓   | ✓        | 关   | 面板显示，**不再两份** |
  | 列路由 + 面板折叠      | ✓   | ✗        | 开   | 抽屉顶上，内容仍可见   |

  **证据**：新增 `GlobalOverlays/acceptancePortalHost.test.tsx`，四个用例按**渲染计数**断言上表
  （`Drawer` 与 `PortalContent` 做桩，使计数只反映抽屉自身的开合判定）。反向验证过：
  把条件改回只判 view type，「列路由 + 面板展开」那条**精确变红**（`expected 1 to be +0`，重复重现），
  其余三条不受影响 —— 说明它测的正是这个条件。

  ⚠️ **写这个测试时我第一版是空过的**：用了字面量 `'Acceptance'`，而真枚举是
  `PortalViewType.Acceptance = 'acceptance'`（小写），于是谓词恒为 false，
...
| 项                    | 值                                                                                                                                           |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 交付头                | 随每次推送推进，因此这里不钉一个提交号；**经 CI 验证 Test CI 全绿的提交清单见 §7.2**。首个被 CI 检查的提交是 `e8c44cb8`，其失败与修复见 §7.6 |
| 远端                  | `origin/feat/task-first-convergence`，与本地 HEAD 一致（已用 `git ls-remote` 核对）                                                          |
| 基线                  | `58d79735`                                                                                                                                   |
| 规模                  | 自基线 `58d79735` 起 82 commits / 594 files，其后追加 5 个修复与取证提交（§7.6、§7.7）                                                       |
...
**修复后的第二次运行**（目标提交 `bb7ccebe`）：

| 工作流  | run                                                              | 结论        | 说明                                                                                                      |
| ------- | ---------------------------------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------- |
| Test CI | <https://github.com/alexj11324/orvilo1/actions/runs/35168692999> | **success** | 12/12 job 全绿，含此前失败的 `Typecheck`、`Test Database`、`Test Server (shard 1/2)`                      |
| E2E CI  | <https://github.com/alexj11324/orvilo1/actions/runs/35168693085> | failure     | `83 scenarios (2 failed, 81 passed)`；总数由 85 降到 83，正是 §7.6.3 退役的那两条，两条 Home 失败随之消失 |

E2E 余下的 2 条仍是 §7.6.4 的文稿场景，**不属本轮**，判据见该节（本轮新增了「失败集合在两次运行间漂移」一条）。

第三次运行（仅文档提交 `3056601f`）：**Test CI 12/12 job 全绿**。此后文档不再逐次登记，因为「交付头」每次推送都会过期 —— 判据放在本节的表里，以 CI 的实际结论为准。
...
| -------------------------------------------------------------------------- | ------ | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `apps/server/.../serverRuntimes/agentDocuments.ts`                         | 10     | `263ebf78`  | 该提交把 `pinToTask` 挪进 `withDocumentOutcome`，并给后者的 `T` 加了约束 `{ documentId?: string } \| undefined`。**关键的一环不是约束本身，而是 `input` 上那个 `T` 型回调**（`getAgentDocumentId?: (result: T) => string \| undefined`）：它让 `T` 出现在**逆变位置**，编译器便无法只从 `operation` 定下 `T`，于是在体内 `attachToTask(result)` 处**退回到 `attachToTask` 的约束**，三元真分支给出的 `doc` 随之被拓宽成 `T \| { documentId?: string } \| undefined`。后果有二：①`input.getAgentDocumentId?.(doc)` 报 TS2345（实参不是 `T`）；②handler 的返回类型变成「service 行类型 `\| { documentId?: string } \| undefined`」，缺 `id` 而不满足 `AgentDocumentsExecutionRuntime` 要求的 `AgentDocumentRecord \| undefined`（TS2322）。此外 `removeDocumentById` 的 outcome 是 `boolean`，根本无法满足该约束 | 恢复 `T` 无约束，把「这个结果能不能 attach」变成真正的运行时类型守卫 `isAttachableDocument`（`removeDocument` 的 outcome 确实是个 boolean，本来就得在运行时判）。`attachToTask` 因此只需返回失败信息，不再搬运 `{ doc }` |
...
**三处机制都用最小探针在本机复现过**，不是靠推断（`bunx tsc --noEmit --strict --target es2022` 单文件）：

- `.filter(类型守卫)` 在 `any[]` 上塌成 `any`：探针**逐字符复现**了 CI 的 `TS7006`；换成 `unknown[]` 后干净。
- `satisfies` 不拓宽：探针逐字符复现了 CI 的 `TS7053`；改成注解后干净。
- 受约束的泛型在第 1 行那条上：探针逐字符复现了 CI 的 `TS2345` 与 `TS2322`。
  ⚠️ 这个过程本身值得记：**第一版探针写错了模型，编译干净通过**，我据此差点把文档里那条机制解释当已验的写下去。补上 `input` 上的 `T` 型回调才复现 —— 也就是说这条的触发条件是**逆变位置的 `T`**，不是「约束本身会把约束当成返回类型」这种更省事的说法。**探针通过只能证明模型对了，不能证明真因找到了。**
...
**第二次运行新增的证据（关键）**：修复后那次 E2E 里，这 2 条的**失败集合发生了漂移** ——
第一次是「通过右键菜单重命名文稿」+「删除文稿」，第二次变成「重命名文稿后按 Enter 确认」+
「删除文稿」。同一区域的场景在两次运行间换人失败，是**顺序依赖 / 不稳定**的特征，
而不是确定性的产品破坏；它们共用的判据「用 UI 新建的文稿在列表里以 `Untitled|无标题` 出现」
本身就依赖前序场景遗留的状态。

**残留不确定性（不掩饰）**：本机无法运行 E2E（需 docker + next + playwright），
所以无法证明它们在**本分支的基点上**就是红的。能确定的是：本分支没有触碰它们的任何
可达路径，两次运行的失败集合还不一致，而 `main` 的处置是删除它们。合并时随 main 的
这两个提交（`bbd6ead3`、`2769eb8c`）一并消失，不构成本分支的交付内容。

**为什么不顺手把它们也退役**：文稿 / 社区的退役是另一个 agent 的在途工作（用户明示
「这个你不管」），且 `main` 已用 `2769eb8c` 删掉这套场景。在这里重复一次，既有与那个
agent 冲突的风险，也等于对他的工作做重复判断。
...
| **CI（已验证的提交）**                                 | **Test CI 12/12 job 全绿**：`bb7ccebe`、`3056601f`（后者仅文档改动）。E2E 83 场景中 2 条失败，均为 §7.6.4 的文稿场景（非本轮，且失败集合在两次运行间漂移） |
...
## 10. 计划各 stage「验收」条款・逐条对照

§9 对的是方案 §15 的 7 条**总体**完成定义。这一节对的是方案 §3–§10 里**每个 stage 各自的验收小节**（共 45 条判据）——
两者粒度不同，只对前者会低估未验证项的数量。

**结论：需要真实产品 / 运行时验证的是 10 条，分布在 S20 / S30 / S40 / S50 / S60 / S70 六个 stage。**
其余均为已满足，且每条的判据都指向可复查的位置。

| stage | 验收条款                                                      | 判定          | 依据                                                                                |
| ----- | ------------------------------------------------------------- | ------------- | ----------------------------------------------------------------------------------- |
| S00   | 当前分支与用户脏改动已记录                                    | ✅            | §0；脏改动范围裁决 §0.3                                                             |
| S00   | 社区 / 文稿删除状态已确认                                     | ✅            | §0.3（属另一 agent；main 已落地 `bbd6ead3`、`2769eb8c`）                            |
| S00   | Agent / 引擎新改动被列为保护项                                | ✅            | §0.1                                                                                |
| S00   | 每个计划删除目录的运行时消费者已定位或明确阻塞                | ✅            | §1.x 各功能域清单（逐目录消费者）                                                   |
| S00   | 不能只提交文档就停止                                          | ✅            | S10–S80 均已实施                                                                    |
| S10   | 退役入口不能从任一客户端菜单 / 命令菜单 / 旧偏好 / 标签页恢复 | ✅            | 注册表 `tier` 单一来源 + 读路径数据闸门（§2.1）+ 标签页丢弃（`e965e3e5`）；各有测试 |
| S10   | 任务入口一致                                                  | ✅            | 侧栏 / 命令菜单 / 移动端 / Electron 菜单同源于注册表；CommandMenu 新增「新建任务」  |
| S10   | 未授权用户不能通过改客户端开关恢复后台能力                    | ✅            | 服务端无条件拒绝（`c95f9ba6`），有测试钉住（§7.6.2）                                |
| S10   | 路由同步测试仍覆盖 Web / Electron 共同部分                    | ✅            | `desktopRouter.sync.test.tsx` 在 CI 绿                                              |
| S20   | 新用户默认看板                                                | ✅            | `DEFAULT_HOME_MODE='task'` + `WebHomeRedirect`                                      |
| S20   | 旧用户有效偏好保留                                            | ✅            | §8.3                                                                                |
| S20   | 个人 / 工作区根路由正确                                       | ✅            | 含保留段修复与同步守卫（`bebd90a8`）                                                |
| S20   | 认证深链接不被抢走                                            | ✅            | §2.2.2（深链接优先，满足 NAV-07）                                                   |
| S20   | **旧首页独有组件不在无关页面加载 / 轮询**                     | ⚠️ **未验证** | 需真实网络面板 / 真机；本机无法观测加载与轮询                                       |
| S20   | **任务抽屉、全局审批和通知目标仍正常**                        | ⚠️ **部分**   | 渲染计数已验（恰好一个宿主，§7.5 HOME-02）；真实界面未验                            |
| S30   | 退役 UI 和专用入口已真正移除                                  | ✅            | 304 条删除清单可核（`task-first-changes.md`）                                       |
| S30   | **保留工具仍执行**                                            | ⚠️ **未验证** | §8.2 只核验了存在性与调用链，**未在运行时执行**                                     |
| S30   | 旧数据可管理                                                  | ✅            | 读路径保留（§8.2）                                                                  |
| S30   | **活动任务无失踪**                                            | ⚠️ **未验证** | 需真实在途任务                                                                      |
| S30   | 服务端拒绝确已退役的业务写操作                                | ✅            | 无条件拒绝 + 测试（§7.6.2）                                                         |
| S30   | 公共访客不能通过旧入口绕过策略                                | ✅            | `execAgent` 为完整 choke point + 拒绝断言                                           |
| S30   | 共享组件和正常任务分享没有被误删                              | ✅            | §8.2 逐条核验（`TopicShareModel`、`shareChat`、`apps/share` 等）                    |
| S40   | **整合前后同一权限与过滤条件得到等价记录 / 总数**             | ⚠️ **未验证** | 需真实数据对照，本机无库访问                                                        |
| S40   | 心跳与周期任务不丢失                                          | ✅            | §8.4：作业全部保留，零服务端删除                                                    |
| S40   | 操作和历史仍可达                                              | ✅            | §2.3                                                                                |
| S40   | **切换视图不会重复轮询或重复触发**                            | ⚠️ **未验证** | 需真实界面交互                                                                      |
| S40   | 时区和已有调度配置不变                                        | ✅            | 未改任何调度配置                                                                    |
| S50   | 项目资料和任务附件能读写                                      | ✅            | PGlite + 客户端测试（§2.6）                                                         |
| S50   | 产物可追溯                                                    | ✅            | 复用既有 `works.originTopicId`，零新增表                                            |
| S50   | 同一文件可以安全复用                                          | ✅            | PGlite                                                                              |
| S50   | 删除引用不误删原文件                                          | ✅            | PGlite 用例（`15d06a28`）                                                           |
| S50   | **私人和跨工作区资源不泄露**                                  | ⚠️ **未验证** | 需真实多用户 / 多工作区                                                             |
| S50   | 旧资料仍有真实入口                                            | ✅            | §2.6                                                                                |
| S50   | **错误可恢复且不会重复收费**                                  | ⚠️ **未验证** | 需真实计费链路                                                                      |
| S60   | 普通任务无需管理独立 Goal                                     | ✅            | §2.7                                                                                |
| S60   | **长任务仍可协调、恢复、查看计划和验收**                      | ⚠️ **未验证** | 运行时协调行为                                                                      |
| S60   | 旧 Goal 不丢失                                                | ✅            | §2.7                                                                                |
| S60   | 规则来源与作用范围明确                                        | ✅            | `AnchorCard` 保留筛选 / 正典 / 越界                                                 |
| S60   | 用户禁用规则后不继续当作启用                                  | ✅            | `listLessonsWithRecent` 按 `status='active'` 过滤（代码级 + 测试）                  |
| S60   | 跨视图任务与会话归属不变                                      | ✅            | §2.7                                                                                |
| S70   | **设置只保留可解释、可用的工作能力**                          | ⚠️ **部分**   | 分组已按能力重排（`3242086a`）；**可用性判断需真机**                                |
| S70   | 真实身份、额度、审计和连接没有被裁掉                          | ✅            | §8.2                                                                                |
| S70   | 中英文主路径一致                                              | ✅            | 三处齐 + 护栏（40 例，反向验证过）                                                  |
| S70   | 产品定位文案无旧功能承诺                                      | ✅            | §2.8                                                                                |
| S70   | 没有因粗暴清依赖破坏保留能力                                  | ✅            | CI 全绿                                                                             |

### 10.1 那 10 条为什么是「未验证」而不是「没做」

它们的**实现都在**（代码、迁移、测试都在分支里），把它们判为未验证的原因是：这些条款的措辞是**行为性**的
——"不在无关页面加载 / 轮询"" 仍执行 ""无失踪"" 等价记录 / 总数 ""不会重复轮询"" 不泄露 ""不会重复收费"" 仍可协调 "
"仍可恢复"—— 只能由**真实运行时或真实产品界面**来证明，本机没有合规手段产生这些证据：

- 方案 §13.1 把质量门禁与验收都放在 GitHub Actions / 已授权测试环境，禁止在本机起重环境绕过。
- §13.3 明文：远端缺少凭据、设备或服务时，**记录 BLOCKED 与缺口**，不要回头在用户电脑上启动重环境。

所以这 10 条**不是 "待补的实现"，而是 "待验的行为"**，与 §7.4 的 BLOCKED 表是同一批缺口的不同粒度切法。

### 10.2 与前一份结论的关系（修正粒度，不修正方向）

先前给的「§15 七条里 5 满足 / 1 部分 / 1 不满足」在 §15 的粒度上成立，但**粒度太粗**：它把上面这 10 条
行为性条款都折叠进了第 5 条。按每个 stage 自己的验收小节来看，未验证的是 10 条，而不是 1 条。
方向（实施已完成、验收未完成）不变，数量需要按这一节为准。
