...
import type { RuntimeMentionedAgent } from '@orvilo/types';
...
export interface AvailablePluginInfo {
   ...
   * 'orvilo-skill' for OrviloSkill providers, 'custom' for user-added custom
   * MCP connectors (aligns with ConnectorSourceType.custom).
   */
  type: 'builtin' | 'composio' | 'orvilo-skill' | 'custom';
}
...
const defaultFormatContext = (context: AgentManagementContext): string => {
  ...
  if (context.availablePlugins && context.availablePlugins.length > 0) {
    ...
    const orviloSkillPlugins = context.availablePlugins.filter((p) => p.type === 'orvilo-skill');
    ...
    if (orviloSkillPlugins.length > 0) {
      const orviloSkillItems = orviloSkillPlugins
        .map((p) => {
          const desc = p.description ? ` - ${escapeXml(p.description)}` : '';
          return `    <plugin id="${p.identifier}">${escapeXml(p.name)}${desc}</plugin>`;
        })
...
<instruction>The user has @mentioned the following agent(s) in their message. You MUST call the \`orvilo-agent-management____callAgent\` tool to delegate the user's request to the mentioned agent. Do NOT attempt to handle the request yourself — call the agent and let them respond.</instruction>
${agentsXml}
</mentioned_agents>`;
};
