...
2. **CDP 实测**（`:9222`，1440×900），**两端按各自形态断言**（初版这里写成「右栏同样」，
   与更正后的右栏 DOM 清单自相矛盾 —— 右栏根本没有 `<a>`）：
   - 主列：里程碑行的 `<a href="#milestone-<id>">` 数量 `0 → ≥1`，且行上有 `id` 落点
   - 右栏：行是**两层 `role=button`**（`tabindex="-1"`），点击导航到项目的 Issues 路由；
     **右栏不该出现 `<a>`**
