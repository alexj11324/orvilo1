...
import { OFFICIAL_URL } from '@orvilo/const';
...
const importDesktopActions = async () => {
  ...
  vi.doMock('@orvilo/const', async (importOriginal) => ({
    ...((await importOriginal()) as typeof OrviloConst),
    isDesktop: true,
  }));
...
describe('resolveBillboardAction', () => {
  ...
  it('should resolve resetOnboarding on the official web origin', () => {
    window.location.href = OFFICIAL_URL;
  ...
  it('should resolve resetOnboarding on desktop synced to an official self-host url', async () => {
    electronState.dataSyncConfig = {
      active: true,
      remoteServerUrl: OFFICIAL_URL,
...
describe('runBillboardAction', () => {
  ...
  it('should reset then open the official web onboarding externally on desktop', async () => {
    ...
    try {
      ...
      expect(resetOnboarding).toHaveBeenCalledTimes(1);
      expect(openExternalLink).toHaveBeenCalledWith(urlJoin(OFFICIAL_URL, '/onboarding'));
    } finally {
      restoreDesktopMock();
    }
  });
