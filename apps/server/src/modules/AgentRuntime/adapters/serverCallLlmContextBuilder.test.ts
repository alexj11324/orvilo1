import type { AgentState, AgentWorldSnapshot, CallLLMPayload } from '@orvilo/agent-runtime';
import type { ResolvedToolSet } from '@orvilo/context-engine';
...
describe('buildServerCallLlmContext - workspace context', () => {
  ...
  it('injects the personal-space origin only when the run has no workspace', async () => {
    ...
    expect(serverMessagesEngineMock).toHaveBeenCalledWith(
      expect.objectContaining({ workspaceContext: { appUrl: 'https://orvilo.aspectlylabs.com' } }),
    );
  });
