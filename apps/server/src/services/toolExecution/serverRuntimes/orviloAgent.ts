...
import { PlanExecutionRuntime } from '@orvilo/builtin-tool-orvilo-agent/planRuntime';
...
import { consumeStreamUntilDone } from '@orvilo/model-runtime';
...
import { parseDataUri } from '@orvilo/utils/uriParser';
...
import { normalizeMultimodalImageItems } from './orviloAgentImage';
import { createServerPlanRuntimeService } from './orviloAgentPlan';
...
interface OrviloAgentRuntimeContext {
  agentId?: string | null;
  /**
...
class OrviloAgentExecutionRuntime {
  ...
  constructor(context: OrviloAgentRuntimeContext) {
...
export const orviloAgentRuntime: ServerRuntimeRegistration = {
  factory: (context) => {
    if (!context.serverDB) {
      throw new Error('serverDB is required for OrviloAgent execution');
    }
    if (!context.userId) {
      throw new Error('userId is required for OrviloAgent execution');
    }
    if (!context.messageId) {
      throw new Error('messageId is required for OrviloAgent execution');
    }

    return new OrviloAgentExecutionRuntime({
