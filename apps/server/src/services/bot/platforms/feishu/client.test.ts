import type * as FeishuAdapterModule from '@orvilo/chat-adapter-feishu';
...
vi.mock('@orvilo/chat-adapter-feishu', async (importOriginal) => ({
...
describe('FeishuWebhookClient.extractFiles', () => {
  ...
  it('caches LarkApiClient across multiple extractFiles calls (token cache hot)', async () => {
    const { LarkApiClient } = await import('@orvilo/chat-adapter-feishu');
