...
import { createOrvilo, DEFAULT_BASE_URL } from './index';
...
describe('createOrvilo', () => {
  it('targets Orvilo Cloud by default with bearer auth', async () => {
    const { fetch, requests } = captureFetch();
    const orvilo = createOrvilo({ apiKey: 'sk-ov-test', fetch });

    const { data, response } = await orvilo.health.check();

    expect(requests[0].url).toBe(`${DEFAULT_BASE_URL}/api/v1/health`);
    expect(requests[0].headers.get('authorization')).toBe('Bearer sk-ov-test');
  ...
  it('serializes JSON bodies from typed resource calls', async () => {
    ...
    await orvilo.agentGroups.create({ body: { name: 'group' } });
  ...
  it('substitutes path params in resource calls', async () => {
    ...
    await orvilo.agents.get({ path: { id: 'agt_123' } });
  ...
  it('preserves per-call Headers instances and tuple arrays on write methods', async () => {
    ...
    await orvilo.agentGroups.create({
      body: { name: 'group' },
      headers: new Headers([['X-Trace', '1']]),
    });
    await orvilo.agents.create({
  ...
  it('normalizes tuple-array headers at client level and on read methods', async () => {
    ...
    await orvilo.agents.list({ headers: [['X-Call', 'b']] });
  ...
  it('drops a client-default Content-Type on form-data uploads', async () => {
    ...
    await orvilo.files.create({ body: { file: new Blob(['x']) } });
  ...
  it('keeps client instances isolated between SDK instances', async () => {
    ...
    createOrvilo({ apiKey: 'sk-ov-a', fetch: a.fetch });
    const clientB = createOrvilo({ apiKey: 'sk-ov-b', baseURL: 'http://b.local', fetch: b.fetch });

    await clientB.users.me();

    expect(a.requests).toHaveLength(0);
    expect(b.requests[0].url).toBe('http://b.local/api/v1/users/me');
    expect(b.requests[0].headers.get('authorization')).toBe('Bearer sk-ov-b');
  });
});
