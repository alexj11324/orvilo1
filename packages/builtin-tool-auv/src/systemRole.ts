...
- Example: ["invoke", "display.list"]. Orvilo adds --json and its private run store automatically.
- Capture commands return artifacts with a local file_path. To actually inspect a returned image, call orvilo-local-system readFile with that exact file_path. If image upload is temporarily unavailable, retry readFile once with the same path. Do not infer image contents from capture metadata.
