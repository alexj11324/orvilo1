---
title: Integrating Orvilo with Ollama for Enhanced Language Models
description: >-
  Learn how to configure and deploy Orvilo to leverage Ollama's powerful
  language models locally. Follow the guide to run Ollama and Orvilo on your
  system.
tags:
  - Ollama integration
  - Orvilo configuration
  - Local deployment
  - Language models
  - Ollama usage
---
...
Ollama is a powerful framework for running large language models (LLMs) locally, supporting various language models including Llama 2, Mistral, and more. Now, Orvilo supports integration with Ollama, meaning you can easily use the language models provided by Ollama to enhance your application within Orvilo.

This document will guide you on how to configure and deploy Orvilo to use Ollama:
...
## Running Orvilo Locally

Assuming you have already started the Ollama service locally on port `11434`. Run the following Docker command to start Orvilo locally:

```bash
docker run -d -p 3210:3210 -e OLLAMA_PROXY_URL=http://host.docker.internal:11434 alexj11324/orvilo1
```

Now, you can use Orvilo to converse with the local LLM.

For more information on using Ollama in Orvilo, please refer to [Ollama Usage](/docs/usage/providers/ollama).
