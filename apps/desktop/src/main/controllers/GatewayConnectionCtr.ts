...
/**
 * Local mirror of `@orvilo/types`' `BuiltinServerRuntimeOutput`. Inlined
 * because the desktop tsconfig doesn't expose `@orvilo/types`, and the shape
 * is tiny + stable.
 */
...
export default class GatewayConnectionCtr extends ControllerModule {
  ...
  /**
   * Platform-specific handlers the shared `@orvilo/device-control` dispatcher
   * delegates to. Git + workspace-scan methods run inside device-control over
   * `@orvilo/local-file-shell`; only file preview / index (and preview
   ...
   * method name. The dispatch logic lives in `@orvilo/device-control` so the
  ...
  private async executeDeviceRpc(method: string, params: unknown): Promise<unknown> {
    const { executeDeviceRpc: runDeviceRpc } = await import('@orvilo/device-control');
    ...
    switch (apiName) {
      ...
      case 'scanHeterogeneousAgents': {
        const { scanHeterogeneousAgentsOnHost } =
          await import('@orvilo/heterogeneous-agents/scanHost');
