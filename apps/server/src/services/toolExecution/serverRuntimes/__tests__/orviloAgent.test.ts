import { MAX_MEDIA_URLS, OrviloAgentIdentifier } from '@orvilo/builtin-tool-orvilo-agent';
...
vi.mock('@orvilo/utils', async (importOriginal) => ({
...
const { orviloAgentRuntime } = await import('../orviloAgent');

describe('orviloAgentRuntime', () => {
  ...
  it('should transcode unsupported images before calling the multimodal model', async () => {
    ...
    const runtime = orviloAgentRuntime.factory(baseContext);
  ...
  it('should have the correct identifier', () => {
    expect(orviloAgentRuntime.identifier).toBe(OrviloAgentIdentifier);
  });

  it('should require serverDB, userId and messageId', () => {
    expect(() => orviloAgentRuntime.factory({ toolManifestMap: {}, userId: 'user-1' })).toThrow(
      'serverDB is required for OrviloAgent execution',
    );

    expect(() => orviloAgentRuntime.factory({ serverDB: {} as any, toolManifestMap: {} })).toThrow(
      'userId is required for OrviloAgent execution',
    );

    expect(() =>
      orviloAgentRuntime.factory({
        serverDB: {} as any,
        toolManifestMap: {},
        userId: 'user-1',
      }),
    ).toThrow('messageId is required for OrviloAgent execution');
  });
  ...
  it('should pass workspaceId when initializing multimodal model runtime', async () => {
    const runtime = orviloAgentRuntime.factory({ ...baseContext, workspaceId: 'workspace-1' });
  ...
  it('should resolve stable refs for durable tool-result images', async () => {
    ...
    const runtime = orviloAgentRuntime.factory({ ...baseContext, topicId: 'topic-1' });
  ...
  describe('todos', () => {
    ...
    it('applies operations against ctx.currentTodos when no plan document exists', async () => {
      ...
      const runtime = orviloAgentRuntime.factory(todoContext);
