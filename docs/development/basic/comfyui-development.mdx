...
description: >-
  Learn how to add new models, workflows, and features to Orvilo's ComfyUI
  integration
tags:
  - ComfyUI
  - Development Guide
...
This guide is based on actual code implementation and helps developers extend Orvilo's ComfyUI integration functionality.

## Architecture Overview

Orvilo ComfyUI integration uses a four-layer service architecture built around the main `OrviloComfyUI` class:

```plaintext
packages/model-runtime/src/providers/comfyui/
├── index.ts                      # OrviloComfyUI main class entry
...
The main `OrviloComfyUI` class initializes four core services:
...
```typescript
// No authentication
const comfyUI = new OrviloComfyUI({
...
```typescript
import { OrviloComfyUI } from '@orvilo/model-runtime';
