...
title: Custom MCP
description: >-
  Connect your own MCP servers to Orvilo — add private APIs, internal tools,
...
The MCP Marketplace offers thousands of ready-to-use integrations, but sometimes you need to connect a private API, an internal company tool, or a custom MCP server you've built yourself. **Custom MCP** lets you add any MCP-compatible server directly to Orvilo without going through the marketplace.
...
### Preset connectors on the Connectors page

**Workspace settings → Connectors** keeps only two kinds of entries: OAuth connectors and a curated **MCP** catalog (GitHub, Linear, Notion, Vercel, Figma, Sentry, PostHog, Supabase, Atlassian, Hugging Face). Click **Connect** on a preset and the same configuration dialog opens pre-filled with the server's identifier, Streamable HTTP endpoint, and the authentication method the provider supports — OAuth 2.0 where the provider allows dynamic client registration, API Key where it does not (GitHub). Adding a preset stores it as a regular custom MCP connector, so it also appears in every other place custom MCPs are managed.

Builtin tools no longer appear on the Connectors page: they are managed in the backend with all permissions granted, and are not user-configurable.
...
<Steps>
  ...
  Orvilo automatically fills in the identifier, connection type, URL or command, and any other detected parameters. Review the auto-filled fields, adjust if needed, then proceed to test the connection.
</Steps>
...
Use this for local MCP servers that run as a process on your machine — command-line tools, scripts, or locally installed packages. **Available on the Orvilo desktop app only; not supported on the web.**
...
Before saving, click **Test connection** to verify that Orvilo can reach your MCP server and fetch its tool list. If the test succeeds, you'll see the available tools listed in the preview panel on the right.
