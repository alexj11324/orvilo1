...
description: >-
  Master Orvilo with keyboard shortcuts — command palette, Agent switching,
...
Orvilo provides a comprehensive set of keyboard shortcuts so you can work without constantly reaching for the mouse. Master a few key ones and your workflow noticeably speeds up — especially when switching Agents, managing conversations, or navigating the interface.
...
These keys work on the Inbox page while focus is outside a text field. They are not customizable in Shortcuts settings, and they do not intercept typing in an editor or the Command Menu.

| Action                              | Shortcut               |
| ----------------------------------- | ---------------------- |
| **Move Inbox selection**            | `J` / `K` or `↓` / `↑` |
| **Open the selected Inbox card**    | `Enter`                |
| **Back from Inbox detail (narrow)** | `Esc`                  |
...
**Tip:** Start with the Command Menu (`⌘ + K` / `Ctrl + K`) — it's the fastest way to jump to any Agent, Topic, task, view, or setting.
