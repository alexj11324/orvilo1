...
export const SUMMARY_DESCRIPTION_PROMPT_VERSION = 'v1';

export const SUMMARY_DESCRIPTION_JSON_SCHEMA = {
  name: 'summary_description',
  schema: {
    additionalProperties: false,
    properties: {
      description: {
        description: 'The concise skill description in the target language',
        type: 'string',
      },
    },
...
export const chainSummaryDescription = (
  ...
  messages: [
    ...
    { content: '{"description": "擅长文创艺术作品起名"}', role: 'assistant' },
    {
      content: `输入: {你是一名创业计划撰写专家，可以提供包括创意名称、简短的标语、目标用户画像、用户痛点、主要价值主张、销售/营销渠道、收入流、成本结构等计划生成。} [en-US]`,
      role: 'user',
    },
    {
      content: '{"description": "Good at business plan writing and consulting"}',
      role: 'assistant',
    },
    {
    ...
    { content: '{"description": "擅长 ts 转换和补充类型声明"}', role: 'assistant' },
    {
      content: `输入: {
用户正常书写面向开发者的 API 用户使用文档。你需要从用户的视角来提供比较易用易读的文档内容。\n\n一个标准的 API 文档示例如下：\n\n\`\`\`markdown
---
title: useWatchPluginMessage
description: 监听获取 Orvilo 发过来的插件消息
nav: API
---\n\n\`useWatchPluginMessage\` 是 Chat Plugin SDK 封装一个的 React Hook，用于监听从 Orvilo 发过来的插件消息。
} [ru-RU]`,
      role: 'user',
    },
    {
      content:
        '{"description": "Специализируется на создании хорошо структурированной и профессиональной документации README для GitHub с точными техническими терминами"}',
      role: 'assistant',
    },
    {
    ...
    { content: '{"description": "擅长创业计划撰写与咨询"}', role: 'assistant' },
