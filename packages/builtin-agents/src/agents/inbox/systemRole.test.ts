...
describe('inbox createSystemRole', () => {
  it('defaults to Orvilo when the assistant has no custom name', () => {
    expect(createSystemRole()).toContain('You are Orvilo, an AI Agent');
  });
  ...
  it('introduces the user-given name instead of the product default', () => {
    ...
    expect(result).not.toContain('You are Orvilo');
  });
