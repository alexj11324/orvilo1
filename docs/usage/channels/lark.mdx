---
title: Connect Orvilo to Lark
description: >-
  Learn how to create a Lark custom app and connect it to your Orvilo agent as
...
# Connect Orvilo to Lark

By connecting a Lark channel to your Orvilo agent, team members can interact with the AI assistant directly on Lark through private chats and group conversations.
...
Orvilo supports two connection modes for Lark bots:
...
For a new bot, use the agent app shortcut on the Lark Open Platform. It presets permissions and events for an agent app. This creates the Lark app; you still choose the agent it connects to in Orvilo.
...
<Steps>
  ...
  ![Lark Open Platform agent app banner with the Create button](/images/docs/channels/lark-agent-entry.png)

  ### Choose an Avatar and Name

  On **Create a Lark app for your agent**, select an **Avatar** and enter a **Name**, such as "Orvilo Weekly Helper". Click **Create** and wait for the automatic configuration to finish.

  ![Lark agent app creation form with an avatar and the name Orvilo Weekly Helper](/images/docs/channels/lark-agent-create.png)
  ...
  ![Lark App created page showing App ID with App Secret hidden](/images/docs/channels/lark-agent-created.png)

  ### Connect to Orvilo

  Open the agent you want to connect in Orvilo, then go to **Settings** → **Channels** → **Lark**. Enter the **App ID** and **App Secret** you copied. Keep the default **WebSocket** connection mode under **Advanced Settings** and click **Save Configuration**.
  ...
  Follow the Lark page prompts to confirm the app is published and available. Complete administrator approval first if your organization requires it. Click **Open App** and send the bot a message to confirm that your Orvilo agent replies.

  If it does not reply, check that the bot capability is enabled, the event subscription uses a long connection, `im.message.receive_v1` is included, and the channel configuration has been saved in Orvilo. See Troubleshooting below for further checks.
</Steps>

The screenshots show the international Lark interface at open.larksuite.com with an example app named "Orvilo Weekly Helper". Use your own app name.
...
<Steps>
  ...
  Click **Create Enterprise App**. Fill in the app name (e.g., "Orvilo Assistant"), description, and icon, then submit the form.
...
## Step 3: Configure Lark in Orvilo

<Steps>
  ### Open Channel Settings

  In Orvilo, navigate to your agent's settings, then select the **Channels** tab. Click **Lark** from the platform list.
...
Back in Orvilo's channel settings, click **Test Connection** to verify the credentials. Then find your bot on Lark by searching its name and send it a message to confirm it responds.
...
To get it: DM the bot once and inspect the inbound event payload — the `open_id` field on the sender is yours. The Lark Developer Portal also exposes a **User ID** lookup that maps mobile/email to `open_id`. Paste it into **Your Platform User ID** in Orvilo's Advanced Settings.
...
- **Bot not connecting (WebSocket mode):** Verify the App ID and App Secret are correct and that the Orvilo service can reach the Lark Open Platform.
...
- **Test Connection failed:** Double-check the App ID and App Secret. Make sure you selected "Lark" (not "飞书") in Orvilo's channel settings.
