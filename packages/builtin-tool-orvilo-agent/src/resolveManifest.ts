import type { BuiltinManifestResolver } from '@orvilo/types';
...
/**
 * Context-aware manifest for the orvilo-agent tool.
 *
 * `orvilo-agent` bundles plan / todo / media-analysis APIs together with the
 ...
 */
export const resolveOrviloAgentManifest: BuiltinManifestResolver = (context) => {
  const inGroup = context.scope === 'group' || context.scope === 'group_agent';
  const hideSubAgentDispatch = inGroup || context.isSubAgent === true;

  if (!hideSubAgentDispatch) return OrviloAgentManifest;

  return {
    ...OrviloAgentManifest,
    api: OrviloAgentManifest.api.filter((api) => api.name !== OrviloAgentApiName.callSubAgent),
