...
const ENTRY_ID = '\0orvilo-icon-imports-fixture.mjs';

const fixturePlugin: Plugin = {
  name: 'orvilo-icon-imports-fixture',
  load(id) {
  ...
  resolveId(id) {
    if (id === 'virtual:orvilo-icon-imports-fixture') return ENTRY_ID;
  },
};

describe('orviloIconImports', () => {
    ...
    const result = await build({
      build: {
        ...
        rolldownOptions: {
          ...
          input: 'virtual:orvilo-icon-imports-fixture',
        },
        write: false,
      },
      configFile: false,
      logLevel: 'silent',
      plugins: [fixturePlugin, ...orviloIconImports()],
    });
