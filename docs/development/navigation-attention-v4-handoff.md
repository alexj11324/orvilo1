...
- 视图 / 列表面再对齐 Linear：SavedView 详情页不再裸露 query JSON——owner 的名称 / 查询 / 布局 / 可见性表单收进 header 的「Edit view」折叠面板（结果页是默认态）；`WorkQueryResults` 列表行换成共享 `AgentTaskItem` 富行（identifier + 状态图标 + 标题 + chips + 头像 + 日期），平铺结果按 `STATUS_KANBAN_COLUMNS` 桶成分组（图标 + 计数 + ChevronDown 折叠头，`WorkQueryStatusGroup`）；Inbox 行加类型圆形 glyph（mention→AtSign、task\_assigned→CircleUserRound、acp\_permission→KeyRound、transfer→ArrowLeftRight、review→GitPullRequest、默认 Bell），替代 Linear 的 avatar+type badge 角色
