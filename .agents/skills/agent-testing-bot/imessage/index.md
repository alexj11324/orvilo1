...
- The **iMessage Desktop branch** checked out (the `imessageBridge` IPC group
  and `@orvilo/chat-adapter-imessage` must be compiled into the main bundle).
  ...
  main build fails to resolve `@orvilo/chat-adapter-imessage`.
...
- **Restart to load main-process fixes.** Editing `imessageBridgeSrv.ts` /
  `@orvilo/chat-adapter-imessage` needs `electron-dev.sh restart` — main isn't
