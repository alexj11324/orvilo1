...
async function main() {
  ...
  console.log(`[@orvilo/memory-user-memory/benchmarks/locomo] loading ${benchmarkLoCoMoFile}`);
  ...
  console.log(
    `[@orvilo/memory-user-memory/benchmarks/locomo] ingesting ${payloads.length} samples to ${baseUrl} (one user per sample)`,
  );
  ...
  for (const payload of payloads) {
    ...
    try {
      console.log(
        `[@orvilo/memory-user-memory/benchmarks/locomo] ingesting sample ${payload.sampleId} (${payload.sessions.length} sessions) for user ${userId}`,
      );

      const res = await post('/api/webhooks/memory-extraction/benchmark-locomo', body);
      console.log(
        `[@orvilo/memory-user-memory/benchmarks/locomo] ingested sample ${payload.sampleId} -> insertedParts=${res.insertedParts ?? 'n/a'} memories=${res.extraction?.memoryIds?.length ?? 0} traceId=${res.extraction?.traceId ?? 'n/a'}`,
      );
    } catch (err) {
      console.error(
        `[@orvilo/memory-user-memory/benchmarks/locomo] failed sample ${payload.sampleId}`,
        err,
      );
      break;
    }
  ...
  console.log(
    `[@orvilo/memory-user-memory/benchmarks/locomo] users used (${usedUserIds.size}): ${[
      ...usedUserIds,
    ].join(', ')}`,
  );
}
