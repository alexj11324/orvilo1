...
describe('Manifest', () => {
  ...
  describe('_getScreenshot', () => {
    ...
    it('should return correct screenshot object for narrow form factor', () => {
      ...
      // `src` mirrors `_getImage`: the branding logo wins over the passed url
      // whenever the deployment configures one, so it is derived from
      // BRANDING_LOGO_URL instead of being hardcoded.
      expect(result).toMatchObject({
        cache_busting_mode: 'query',
        form_factor: 'narrow',
        immutable: 'true',
        max_age: 31536000,
        sizes: '320x569',
        src: qs.stringifyUrl({ query: { v: 1 }, url: BRANDING_LOGO_URL || screenshot.url }),
        type: 'image/png',
      });
    });

    it('should fall back to the narrow default size when the screenshot declares none', () => {
      const screenshot = {
        form_factor: 'narrow' as const,
        url: 'https://example.com/screenshot.png',
      };
      ...
      expect(result.sizes).toBe('640x1138');
    });
  });
});
