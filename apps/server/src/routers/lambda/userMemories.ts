import { BRANDING_PROVIDER, ENABLE_BUSINESS_FEATURES } from '@orvilo/business-const';
import {
...
} from '@orvilo/memory-user-memory';
import type { QueryTaxonomyOptionsResult, SearchMemoryResult } from '@orvilo/types';
import { LayersEnum, queryTaxonomyOptionsSchema, searchMemorySchema } from '@orvilo/types';
...
const searchUserMemories = async (
  ...
  const modelRuntime = await initModelRuntimeFromDeploymentConfig(ctx.userId, provider);
...
const getEmbeddingRuntime = async (userId: string) => {
  const { provider, model: embeddingModel } =
    getServerDefaultFilesConfig().embeddingModel || DEFAULT_USER_MEMORY_EMBEDDING_MODEL_ITEM;
...
export const userMemoriesRouter = router({
  ...
  reEmbedMemories: memoryWriteProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        const options = input ?? {};
        const { agentRuntime, embeddingModel } = await getEmbeddingRuntime(ctx.userId);
