...
describe('assertRoomAccess (integration)', () => {
  ...
  it('lets an active member authorize the workspace room', async () => {
    await expect(
      assertRoomAccess(
        ...
        { id: workspaceId, scope: 'workspace' },
      ),
    ).resolves.toEqual({});
  });
  ...
  it('rejects a member naming a different workspace room', async () => {
    await expect(
      assertRoomAccess(
        db,
        { userId: memberId, workspaceId },
        { id: otherWorkspaceId, scope: 'workspace' },
      ),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
  });
  ...
  const insertProject = async (params: {
    ...
    await db
      .insert(agents)
      .values({ id: agentId, userId: params.ownerId, workspaceId: params.workspaceId });
  ...
  it("lets a member join a public project room but not another member's private project", async () => {
    ...
    await expect(
      assertRoomAccess(
        db,
        { userId: memberId, workspaceId },
        { id: publicProject.id, scope: 'project' },
      ),
    ).resolves.toEqual({ projectId: publicProject.id });
    ...
    await expect(
      assertRoomAccess(
        db,
        { userId: memberId, workspaceId },
        { id: privateProject.id, scope: 'project' },
      ),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
    await expect(
      assertRoomAccess(
    ...
    ).resolves.toEqual({ projectId: privateProject.id });
  });

  it('rejects a project room from another workspace', async () => {
    const foreignProject = await insertProject({
      ownerId: outsiderId,
    ...
    await expect(
      assertRoomAccess(
        db,
        { userId: memberId, workspaceId },
        { id: foreignProject.id, scope: 'project' },
      ),
    ).rejects.toMatchObject({ code: 'NOT_FOUND' });
  });
  ...
  it("lets a member join a public task room but not another member's private task", async () => {
    ...
    const privateTask = await insertTask({
      creatorId: ownerId,
      visibility: 'private',
      workspaceId,
    });
