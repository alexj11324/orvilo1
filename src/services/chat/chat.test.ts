...
vi.mock('@orvilo/fetch-sse', async (importOriginal) => {
...
vi.mock('@orvilo/utils/url', () => ({
  isDesktopLocalStaticServerUrl: vi.fn(),
}));
vi.mock('@orvilo/utils/imageToBase64', () => ({
  imageUrlToBase64: vi.fn(),
}));
vi.mock('@orvilo/utils/uriParser', () => ({
  parseDataUri: vi.fn(),
}));
  ...
  describe('createAssistantMessage', () => {
    ...
    describe('should handle content correctly for vision models', () => {
      it('should include image content when with vision model', async () => {
        ...
        const { parseDataUri } = await import('@orvilo/utils/uriParser');
        const { isDesktopLocalStaticServerUrl } = await import('@orvilo/utils/url');
...
<!-- END SYSTEM CONTEXT -->`,
    ...
    describe('local image URL conversion', () => {
      ...
      it('should convert local image URLs to base64 and call processImageList', async () => {
        const { imageUrlToBase64 } = await import('@orvilo/utils/imageToBase64');
        const { parseDataUri } = await import('@orvilo/utils/uriParser');
        const { isDesktopLocalStaticServerUrl } = await import('@orvilo/utils/url');
...
<!-- END SYSTEM CONTEXT -->`,
  ...
  describe('getChatCompletion', () => {
    ...
    beforeEach(async () => {
      ...
      const { fetchSSE } = await import('@orvilo/fetch-sse');
    ...
    it('should preserve the topic ID when using the browser runtime', async () => {
      ...
      const runtime = await import('@orvilo/model-runtime');
