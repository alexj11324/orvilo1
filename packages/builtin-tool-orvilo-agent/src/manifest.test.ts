...
describe('OrviloAgentManifest', () => {
  it('should keep the package metadata generic for future Orvilo Agent capabilities', () => {
    expect(OrviloAgentManifest.meta.avatar).toBe('🤖');
    expect(OrviloAgentManifest.meta.description).toBe(
      'Run built-in Orvilo Agent capabilities: plan + todo management, sub-agent dispatch, and multimodal media analysis.',
    );
    expect(OrviloAgentManifest.meta.readme).toContain(
      'built-in assistant capabilities that can be expanded over time',
    );
  });

  it('should describe multimodal analysis as a fallback tool', () => {
    const apiDescription = OrviloAgentManifest.api[0].description;
  ...
  it('should instruct agents to prefer native multimodal access before media analysis', () => {
    expect(OrviloAgentManifest.systemRole).toContain('`analyzeMedia` is only a fallback');
    expect(OrviloAgentManifest.systemRole).toContain(
  ...
  it('should route local media through file refs instead of local URLs or base64 text', () => {
    const analyzeMedia = OrviloAgentManifest.api[0];
    ...
    expect(OrviloAgentManifest.systemRole).toContain('stable ref');
  });

  it('should keep media analysis parameters compatible with strict tool schema validators', () => {
    const parameters = OrviloAgentManifest.api[0].parameters;
  ...
  it('should expose a restrained vent API for reporting platform friction', () => {
    const ventApi = OrviloAgentManifest.api.find((api) => api.name === OrviloAgentApiName.vent);
    ...
    expect(OrviloAgentManifest.systemRole).toContain('<vent>');
  });
});
