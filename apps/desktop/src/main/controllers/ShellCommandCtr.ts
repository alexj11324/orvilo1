...
import type { SandboxCapability } from '@orvilo/device-sandbox';
import type {
...
import {
  ...
  ShellProcessManager,
} from '@orvilo/local-file-shell/shell';
...
export default class ShellCommandCtr extends ControllerModule {
  ...
  private probeSandbox(): Promise<SandboxCapability> {
    return (this.sandboxCapability ??= (async () => {
      try {
        const { probeSandboxCapability } = await import('@orvilo/device-sandbox');
  ...
  async getSandboxCapability(): Promise<DeviceSandboxCapabilityResult> {
    ...
    const { canInstallSandbox } = await import('@orvilo/device-sandbox');
    return {
  ...
  async installSandbox(): Promise<DeviceSandboxInstallResult> {
    const { installDeviceSandbox } = await import('@orvilo/device-sandbox');
  ...
  async handleRunCommand(params: RunCommandParams): Promise<RunCommandResult> {
    ...
    const { createLocalSandboxPolicy } = await import('@orvilo/device-sandbox');
