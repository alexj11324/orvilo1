...
import { BRANDING_PROVIDER, SOCIAL_URL } from '@orvilo/business-const';
...
const Nav = memo<NavProps>(({ mobile, setActiveTab, activeTab = ProviderNavKey.Overview }) => {
  ...
  return mobile ? (
    ...
    <Flexbox horizontal align={'center'} className={styles.nav} justify={'space-between'}>
      ...
      >
        {/* A white-label deployment may have no community server: drop the
            entry rather than render a link that goes nowhere. */}
        {SOCIAL_URL.discord && (
