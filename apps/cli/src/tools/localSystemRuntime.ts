import type { BaseFileSearch, GlobFilesParams, SearchFilesParams } from '@orvilo/local-file-shell';
import { createFileSearchModule } from '@orvilo/local-file-shell';
 ...
 * `@orvilo/types`' `BuiltinServerRuntimeOutput`: `content` is the formatted
...
/**
 * Adapter wiring the CLI's `@orvilo/local-file-shell` functions (file ops) and
