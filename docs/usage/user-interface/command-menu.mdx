...
title: Command Menu
description: >-
  The quick action center of Orvilo — search Agents, Topics, tasks, views,
  settings, and jump anywhere with a few keystrokes.
tags:
  - Orvilo
  - Command Menu
  - Quick Navigation
...
The Command Menu is Orvilo's quick action center. Press `⌘ + K` (Mac) or `Ctrl + K` (Windows/Linux) and a search overlay appears. Type a few keywords to find Agents, Topics, tasks, projects, views, settings, or actions — no need to click through menus or the sidebar. It's the fastest way to navigate Orvilo.
...
| **Work**         | Tasks, projects, teams, and saved views you can read                   |
...
Prefix a query with `type:task`, `type:project`, `type:team`, or `type:savedView` (or `is:` instead of `type:`) to search only that kind of work. Opening the menu also lists recent visits — pages you opened, not a "recently updated" feed. In personal mode, teams are omitted from recents and work search.

See [Inbox and My issues](/docs/usage/getting-started/work) for the Inbox, My issues, Views, and Teams pages those results open.
...
- **Tasks, projects, teams, views** — Jump to work you can already read
