...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to Orvilo's
...
This guide walks through the steps to add a new bot platform to Orvilo's channel system. The platform architecture is modular — each platform is a self-contained directory under `apps/server/src/services/bot/platforms/`.
...
- `packages/chat-adapter-feishu` — Feishu/Lark adapter (`@orvilo/chat-adapter-feishu`)
- `packages/chat-adapter-qq` — QQ adapter (`@orvilo/chat-adapter-qq`)

Each adapter package follows this structure:

```
packages/chat-adapter-<platform>/
├── package.json        # name: @orvilo/chat-adapter-<platform>
├── tsconfig.json
├── tsup.config.ts
└── src/
...
export const myPlatform: PlatformDefinition = {
  ...
  documentation: {
    ...
    setupGuideUrl: 'https://orvilo.aspectlylabs.com/docs/usage/channels/<platform',
  },
  schema,
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in Orvilo → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.
