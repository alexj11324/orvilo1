---
title: Configuring Casdoor Authentication for Orvilo
description: >-
  Learn how to configure Casdoor SSO for Orvilo, including creating an
...
<Steps>
  ...
  | `AUTH_SSO_PROVIDERS`     | Required | SSO provider for Orvilo. Use `casdoor` for Casdoor                            |
  ...
  Configure Casdoor [Webhook](https://www.casdoor.org/docs/webhooks/overview#setting-up-a-webhook) to sync user data updates to Orvilo.
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Casdoor and use Orvilo.
</Callout>

## Docker Compose Deployment with Casdoor

If you're deploying Orvilo using Docker Compose, refer to the following configuration to integrate Casdoor as an authentication service.
...
1. Orvilo needs to communicate with Casdoor, so you need to configure Casdoor's Issuer:

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

This configuration affects Orvilo's login authentication service. Ensure the Casdoor service URL is correct.

2. Allow callback URL in Casdoor to point to Orvilo:

In Casdoor's Web panel under `Authentication -> Applications` -> `<Application ID, default is app-built-in>` -> `Redirect URLs`, add:

```
https://orvilo.example.com/api/auth/callback/casdoor
```
...
```log
orvilo      | [auth][error] r3: "response" is not a conform Authorization Server Metadata response (unexpected HTTP status code)
```
...
```log
orvilo      | [auth][error] TypeError: fetch failed
```

Cause: Orvilo cannot access the authentication service.

Solution:

- Check if your authentication service is running properly and if Orvilo's network can reach it.

- Direct troubleshooting: Use `curl` in the Orvilo container terminal to access `https://auth.example.com/.well-known/openid-configuration`. If JSON data is returned, your authentication service is working correctly.
