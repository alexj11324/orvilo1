...
export interface OrviloBuiltinMcpServerOptions {
  /**
 ...
 */
export class OrviloBuiltinMcpServer {
  ...
  constructor(private readonly options: OrviloBuiltinMcpServerOptions = {}) {
  ...
  get url(): string {
    if (!this.startedUrl) {
      throw new Error('OrviloBuiltinMcpServer not started yet — call start() first');
    }
  ...
  registerOperation(operationId: string, bridge?: AskUserBridge): AskUserBridge {
    if (this.operations.has(operationId)) {
      throw new Error(`OrviloBuiltinMcpServer: operation already registered: ${operationId}`);
    }
