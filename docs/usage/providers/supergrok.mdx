---
title: Using a SuperGrok Subscription in Orvilo
description: >-
  Learn how to connect a SuperGrok or eligible X Premium subscription to Orvilo
...
# Using SuperGrok in Orvilo

[SuperGrok](https://x.ai/pricing) is xAI's subscription plan for higher Grok usage limits and access to frontier models. Orvilo's SuperGrok provider lets you use an eligible SuperGrok or X Premium subscription by signing in to your xAI account—no separate xAI API key is required.

xAI also uses this subscription-based OAuth flow in third-party integrations such as [OpenCode](https://x.ai/news/grok-opencode). Orvilo uses the device authorization flow: it opens xAI in your browser, displays a short code when needed, and waits for you to approve access.

<Callout type={'info'}>
  `SuperGrok` and `xAI` are separate providers in Orvilo. Choose `SuperGrok` to use an existing subscription through OAuth. Choose `xAI` if you want to use an API key from the xAI Console and pay for metered API usage.
</Callout>
...
- Permission to manage AI provider credentials in your Orvilo account or workspace
- For self-hosted Orvilo, outbound access from the server to `auth.x.ai` and `api.x.ai`
...
## Connect SuperGrok to Orvilo

<Steps>
  ### Step 1: Open the SuperGrok provider

  - Open `Settings` in Orvilo
  - Select `AI Providers`
  ...
  Click `Connect with SuperGrok`. Orvilo requests a one-time device code and normally opens the xAI authorization page in a new browser tab.

  If the page does not open automatically, click `Open browser to authorize`. Orvilo also shows the authorization URL and a short code that you can copy manually.

  ### Step 3: Approve access with xAI

  - Sign in with the xAI account that owns your SuperGrok or eligible X Premium subscription
  - Enter the code shown in Orvilo if xAI asks for it
  - Review and approve the authorization request
  - Return to Orvilo and keep the provider page open while it finishes checking the authorization status

  Orvilo will show `Connected` after xAI completes the authorization. The OAuth access and refresh tokens are saved in Orvilo's encrypted provider credential vault and refreshed automatically when necessary.
  ...
  Open the model selector in a conversation and choose a model under `SuperGrok`. Orvilo currently enables `grok-4.5` by default and can load additional models made available to your account.
...
The SuperGrok provider is intended for Grok chat models. Depending on the selected model, Orvilo can use:
...
<Callout type={'warning'}>
  SuperGrok in Orvilo currently supports chat models only. Grok Imagine image or video generation and Grok Voice are separate products and are not exposed through this provider. Use the appropriate xAI API provider or the official Grok app for those capabilities.
</Callout>
...
| Orvilo browser requests | Disabled; requests run through the server   | Depends on the provider configuration |
...
Disconnecting removes the stored SuperGrok OAuth credentials from Orvilo. To use another account, disconnect first and then run `Connect with SuperGrok` again in a browser session signed in to the desired xAI account.

## Troubleshooting

### The authorization page did not open

- Allow pop-ups for your Orvilo site
...
### Orvilo shows 401 or 403 errors after connecting
