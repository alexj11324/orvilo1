...
// dependency on `@orvilo/local-file-shell`). The shape mirrors the
 ...
 * Mirrors `WorkspaceInstructions` in `@orvilo/types` (kept local — this is a
 * leaf package with no `@orvilo/types` dependency).
 */
 ...
 * in `@orvilo/types`; the server narrows `skills` to `ProjectSkillMeta` when
