...
export class Ld {
  ...
  genOrganization() {
    // Resolved against the site origin so the structured data points at a real
    // file. BRANDING_LOGO_URL is what the app itself serves, so the JSON-LD logo
    // and the ProductLogo cannot drift apart.
    const logoUrl = new URL(BRANDING_LOGO_URL, OFFICIAL_SITE).href;

    return {
      '@id': this.getId(OFFICIAL_URL, '#organization'),
      '@type': 'Organization',
      'alternateName': BRANDING_NAME,
      // A ContactPoint with no address is worse than none — omitted rather than
      // emitted with an undefined email.
      'contactPoint': BRANDING_EMAIL.support
        ? {
            '@type': 'ContactPoint',
      ...
      // different organization. A deployment supplies its own or omits it.
      'image': logoUrl,
      'logo': {
        '@type': 'ImageObject',
        'height': 512,
      ...
      'name': BRANDING_NAME,
      // Unset social accounts are dropped: a `null` in `sameAs` is a broken
      // reference to a JSON-LD consumer, not an absent one.
