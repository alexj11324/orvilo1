import type {
  ...
  GrepContentParams,
  GrepContentResult,
} from '@orvilo/local-file-shell/content-search';
 ...
 * sunk into the shared `@orvilo/local-file-shell` package.
 */
export default class ContentSearchService extends ServiceModule {
  ...
  private getImpl(): Promise<BaseContentSearch> {
    this.implPromise ??= import('@orvilo/local-file-shell/content-search').then(
