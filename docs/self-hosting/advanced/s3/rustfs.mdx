...
<Steps>
  ...
  Click `Object Storage` in the left sidebar, then the `Create Bucket` button in the top-right corner to create a new bucket. This example uses the name `orvilo`. Leave Versioning and Object Lock disabled (default settings).
  ...
  | --------------------------- | ------ | -------- |
  | `orvilo-s3-api.example.com` | `9000` | Yes      |
  | `orvilo-s3-ui.example.com`  | `9001` |          |

  After completing the reverse proxy, remember to configure the corresponding SSL certificate and enable HTTPS access.

  ### Set Environment Variables

  Update the Orvilo `.env` file with the following environment variables to use RustFS as the S3 backend:
  ...
  # RustFS API endpoint
  S3_ENDPOINT=https://orvilo-s3-api.example.com
  # Bucket name
  S3_BUCKET=orvilo
