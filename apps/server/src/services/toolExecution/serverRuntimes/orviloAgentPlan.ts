import { type PlanDocument, type PlanRuntimeService } from '@orvilo/builtin-tool-orvilo-agent';
import { AGENT_DOCUMENT_SOURCE_TYPE, AGENT_PLAN_FILE_TYPE } from '@orvilo/const';
 ...
 *
 * The factory is consumed by `orviloAgent.ts`'s `OrviloAgentExecutionRuntime`,
 * which folds plan/todo execution into the orvilo-agent server runtime so the
