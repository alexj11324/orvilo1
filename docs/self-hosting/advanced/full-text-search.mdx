...
description: Choose and operate the pg_search or Elasticsearch backend for Orvilo product search.
tags:
  - Full-Text Search
  - Elasticsearch
  - pg_search
...
Orvilo uses full-text search for product data such as agents, topics, messages, files, knowledge
bases, chat groups, and memories. This is separate from the web-search tools that an agent can call.

Orvilo supports two product-search backends:
...
`pg_search` and `elasticsearch`; the default is `pg_search`. Orvilo does not silently fall back to
...
<Callout type="warning">
  ...
  `pg_search`. The current Elasticsearch migration workflow is intended for an existing Orvilo
...
For a self-managed database, the Orvilo Docker examples use the
`paradedb/paradedb:latest-pg17` image and preload `pg_search`. Apply the normal Orvilo database
migrations to install the extension and the Orvilo-managed BM25 indexes, then keep:
...
applications that still depend on it will stop working after removal. Existing Orvilo deployments
...
sync commands and the same `ES_INDEX_NAMESPACE`; they differ only in how Orvilo reaches the
cluster.

#### External Elasticsearch (Elastic Cloud or a managed cluster)

- an Elasticsearch project reachable from the Orvilo server over HTTPS;
...
Plain HTTP is accepted only for loopback addresses during local development. Orvilo refuses to
...
`elasticsearch` service plus the backfill and sync commands built from the official Orvilo image.
...
follow-up ships, every Orvilo database, including a fresh Docker Compose install, must still run on
...
Orvilo hydrates and authorizes the results against PostgreSQL before returning them.
...
| `fts-search-reindex` | `elasticsearch-reindex` | One-off backfill / status command from the `alexj11324/orvilo1` image; its checkpoint lives in the `fts-search-reindex-state` volume          |
| `fts-search-sync`    | `elasticsearch-sync`    | Long-running incremental consumer from the `alexj11324/orvilo1` image; restarts and logs when it hits failed or dead-letter work              |
...
4. **Switch explicitly.** When `docker compose run --rm fts-search-reindex --status` shows `pending`,
   ...
   ```bash
   docker compose up -d orvilo
   ```

   Nothing switches the provider automatically. Rolling back means restoring
   `FTS_SEARCH_PROVIDER=pg_search` and recreating `orvilo`; the sync worker may keep running.
...
Elastic Cloud path is unchanged: without this variable Orvilo still requires HTTPS and an API key.
...
Do not reuse one namespace for unrelated Orvilo deployments. Do not change the namespace while a
migration or incremental consumer is active.

## Monitor Elasticsearch search

Orvilo exports bounded OpenTelemetry metrics and traces without recording raw queries, user IDs,
