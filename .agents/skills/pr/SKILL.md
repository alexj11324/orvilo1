...
- **A new shared package goes with its consumer**, not the server, unless the server imports it too. A `@orvilo/*` package imported only by desktop/CLI ships in the client PR. Don't carry an unused package in the lower PR.
...
Filter to your touched files — this repo's standalone type-check emits pre-existing env noise (`__ELECTRON__`, `@/types/llm`, unbuilt `@orvilo/types`) that isn't yours.
