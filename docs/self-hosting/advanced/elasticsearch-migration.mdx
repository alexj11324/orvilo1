...
description: Migrate search for a Orvilo deployment using Docker and Neon.
tags:
  - Elasticsearch
  - Search
  - Migration
...
If you deploy Orvilo with Docker, use Neon for your database, and currently rely on `pg_search`, follow this guide to move search to Elasticsearch. Complete the migration before Neon disables `pg_search` for your instance.
...
| `KEY_VAULTS_SECRET`       | Reuse the value from your existing Orvilo application; do not generate a new one              |
...
| `ES_INDEX_NAMESPACE`      | An index prefix, such as `orvilo`; use the same value for the application and migration tools |
...
```bash
export ORVILO_MIGRATION_IMAGE='alexj11324/orvilo1:<deployment-version-tag>'
docker pull "$ORVILO_MIGRATION_IMAGE"
docker volume create orvilo-search-state
```
...
docker run --rm --env-file .env \
  "$ORVILO_MIGRATION_IMAGE" /app/docker.cjs
```
...
docker run --rm --env-file .env \
  ...
  -v orvilo-search-state:/app/.elasticsearch-reindex \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-elasticsearch-reindex.cjs \
  --apply --fresh-run --yes
```

Migration progress is saved in the `orvilo-search-state` volume. Keep this volume. Use `--fresh-run` only for the initial import; omit it when resuming after an interruption.
...
docker run --rm --env-file .env \
  -e ES_REINDEX_STATE_DIR=/app/.elasticsearch-reindex \
  -v orvilo-search-state:/app/.elasticsearch-reindex \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-elasticsearch-reindex.cjs --status
```
...
```bash
docker run -d --name orvilo-fts-search-sync --restart unless-stopped \
  --env-file .env \
  -e FTS_SEARCH_SYNC_ENABLED=true \
  -e MIGRATION_DB=1 \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-elasticsearch-sync.cjs \
...
```bash
docker logs --tail 50 orvilo-fts-search-sync
```
...
Mount the same progress volume on your Orvilo application container with `-v orvilo-search-state:/app/.elasticsearch-reindex` and set `ES_REINDEX_STATE_DIR=/app/.elasticsearch-reindex`. In the application's deployment configuration, set the same Elasticsearch connection details and index prefix, and change `FTS_SEARCH_PROVIDER` to `elasticsearch`. Recreate the application container using the same image version. Editing only the migration's `.env` file does not update the running application.
...
```bash
docker run --rm --env-file .env \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-pg-search-cleanup.cjs --status
docker run --rm --env-file .env \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-pg-search-cleanup.cjs --apply --yes
```
...
The optional search capture installer records its schema version in PostgreSQL. Database migrations must run before capture installation. An older application version must not install over newer capture definitions; if the installer reports an unknown or partial definition, keep the current version running and resolve the mismatch before retrying. The capture upgrade holds source-table writes while replacing definitions, so it does not create a missed-change window or require a full reindex by itself. Existing indexed tool messages require a separate, version-aware cleanup after all sync workers use the new eligibility rule.

For an existing Elasticsearch installation, first upgrade every application and continuous-sync worker to the version that excludes tool and blank messages. Then preview the ineligible PostgreSQL source count and separately record the Elasticsearch `messages` alias's live `role=tool` count:

```bash
docker run --rm --env-file .env \
  -v orvilo-search-state:/app/.elasticsearch-reindex \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-ineligible-message-cleanup.cjs --status
```

Schedule the separate cleanup only after every sync worker is upgraded. Run bounded batches with the same volume mounted for the private checkpoint:

```bash
docker run --rm --env-file .env \
  -v orvilo-search-state:/app/.elasticsearch-reindex \
  "$ORVILO_MIGRATION_IMAGE" /app/fts-search-ineligible-message-cleanup.cjs \
  --apply --yes --all-workers-upgraded \
  --state-file=/app/.elasticsearch-reindex/ineligible-messages.json \
  --batch-size=250 --max-batches=10 --delay-ms=1000
```

Keep the checkpoint volume for resume and do not delete it between runs. A repeated batch is safe: it allocates a newer Outbox revision, and the normal sync worker writes a fenced soft tombstone or the message's current eligible projection. Wait for the Outbox to drain without dead letters, then confirm the Elasticsearch count has decreased toward zero. The command does not change PostgreSQL messages or run Elasticsearch writes itself.

## If the migration is interrupted

Keep the `orvilo-search-state` volume. For an interrupted initial manual import, resolve the error and run the import command again without `--fresh-run`. If automatic migration fails during application startup, resolve the error and any retained lock, then restart the application container. It will continue the backfill, catch-up, and switch.
