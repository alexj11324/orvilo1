...
SESSION="${SESSION:-orvilo-dev}"
AUTH_DIR="${AUTH_DIR:-$HOME/.orvilo-agent-testing}"
STATE_FILE="$AUTH_DIR/web-state.json"
ROOT_ENV_FILE="$REPO_ROOT/.env"
CLI_HOME_NAME="${ORVILO_CLI_HOME:-.orvilo-dev}"
...
check_cli() {
  local api_key="${ORVILO_CLI_API_KEY:-${ORVILO_API_KEY:-}}"
  if [[ -n "$api_key" ]]; then
  ...
  if [[ -f "$CLI_HOME/settings.json" ]] && grep -q "$SERVER_URL" "$CLI_HOME/settings.json" && [[ -f "$CLI_CREDENTIALS_FILE" ]]; then
    ...
    note "cd apps/cli && ORVILO_CLI_HOME=.orvilo-dev bun src/index.ts login --server $SERVER_URL"
    return 1
  fi
}
...
cmd_cli() {
  ...
  ORVILO_CLI_HOME=.orvilo-dev bun src/index.ts login --server "$SERVER_URL"
}
...
export ORVILO_API_KEY=$SEED_API_KEY
export ORVILO_CLI_API_KEY="\${ORVILO_API_KEY}"
export ORVILO_SERVER=$SERVER_URL
...
cmd_cli_seed() {
  ...
  ORVILO_API_KEY="$SEED_API_KEY" ORVILO_CLI_API_KEY="$SEED_API_KEY" check_cli
}
