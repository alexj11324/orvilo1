---
title: Deploy Orvilo with database on Vercel
description: >-
  Learn how to deploy Orvilo with database on Vercel with ease, including:
...
This article will detail how to deploy the server database version of Orvilo on Vercel, including: 1) database configuration; 2) identity authentication service configuration; 3) steps for setting up the S3 storage service.
...
<Steps>
  ...
  Finally, you need to add the `APP_URL` environment variable, which specifies the URL address of the Orvilo application.
</Steps>
...
<Steps>
  ...
  You need to obtain the access key for S3 so that the Orvilo server has permission to access the S3 storage service. In R2, you can configure the access key in the account details:
...
After completing the steps above, the configuration of the server-side database should be done. Next, we can deploy Orvilo to Vercel and then visit your Vercel link to verify if the server-side database is working correctly.
...
To receive future Orvilo updates:

1. **Delete** the current project from Vercel dashboard (Settings → Delete Project)
2. Manually **fork** [alexj11324/orvilo1](https://github.com/alexj11324/orvilo1) on GitHub
...
**Database connection fails** — Verify `DATABASE_URL` format. Ensure `?sslmode=require` is appended for managed databases. For a private CA, set `DATABASE_SSL_CA` to its PEM certificate (literal `\\n` escapes are accepted) so Node verifies the server identity. `psql` does not accept the Node-only `uselibpqcompat` parameter; test it with a URL that omits that parameter and use `sslmode=verify-full&sslrootcert=/path/to/ca.crt`. Check that the database's IP allowlist includes Vercel IPs.
