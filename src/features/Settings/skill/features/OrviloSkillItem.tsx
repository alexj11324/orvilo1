...
import { createOrviloSkillDetailModal } from '@/features/SkillStore/SkillDetail';
...
const OrviloSkillItem = memo<OrviloSkillItemProps>(
  ({ provider, server, isSelected, onSelect, onDelete }) => {
    ...
    const revokeConnect = useToolStore((s) => s.revokeOrviloSkill);
    ...
    const handleDisconnect = () => {
      ...
      confirmModal({
        ...
        content: t('tools.orviloSkill.disconnectConfirm.desc', { name: provider.label }),
        ...
        title: t('tools.orviloSkill.disconnectConfirm.title', { name: provider.label }),
      });
    };
    ...
    const renderStatus = () => {
      if (!server) {
        return (
          <span className={styles.disconnected}>
            {t('tools.orviloSkill.disconnected', { defaultValue: 'Disconnected' })}
          </span>
        );
      }
      ...
      switch (server.status) {
        ...
        case OrviloSkillStatus.ERROR: {
          return <span className={styles.error}>{t('tools.orviloSkill.error')}</span>;
        }
        default: {
          return (
            <span className={styles.disconnected}>
    ...
    const renderAction = () => {
      ...
      if (!server || server.status !== OrviloSkillStatus.CONNECTED) {
        return (
          ...
          items={[
            {
              ...
              key: 'disconnect',
              label: t('tools.orviloSkill.disconnect', { defaultValue: 'Disconnect' }),
      ...
      >
        <Flexbox horizontal align="center" gap={8} style={{ flex: 1, overflow: 'hidden' }}>
          <Flexbox
            ...
            onClick={
              onSelect ? undefined : () => createOrviloSkillDetailModal({ identifier: provider.id })
            }
          >
