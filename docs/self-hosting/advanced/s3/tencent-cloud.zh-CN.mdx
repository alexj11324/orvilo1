...
<Steps>
  ...
  创建存储桶时将指定其名称，下文以 `orvilo` 为例。选择 `公有读私有写`，地域随意，其余配置一概默认即可，然后点击创建。
  ...
  <Callout type={'warning'}>
    请注意，`S3_ENDPOINT` 的值为 `https://cos.ap-chengdu.myqcloud.com`，而不是 `https://orvilo-130xxxxxx2.cos.ap-chengdu.myqcloud.com`
