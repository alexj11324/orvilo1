---
title: Orvilo Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for Orvilo Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest Orvilo image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running Orvilo Container

  Assuming the Orvilo container is named `orvilo`, use the following commands to stop and remove the currently running Orvilo container:
  ...
  ### Pull the Latest Orvilo Image

  Use the following command to pull the latest Docker image for Orvilo:

  ```fish
  docker pull alexj11324/orvilo1
  ```

  ### Restart the Docker Container

  Redeploy the Orvilo container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of Orvilo's chat records are stored in your local browser. Therefore, when redeploying Orvilo using Docker, your chat records will not be lost.
</Callout>
...
<Steps>
  ...
  First, create a `orvilo.env` configuration file with various environment variables, for example:
  ...
  echo "$output" | grep -q "Image is up to date for alexj11324/orvilo1:latest"
  ...
  echo "Detected orvilo update"

  # Remove the old container
  echo "Removed: $(docker rm -f orvilo)"

  # Run the new container(Please change the path to the env file)
  echo "Started: $(docker run -d --network=host --env-file /path/to/orvilo.env --name=orvilo --restart=always alexj11324/orvilo1)"

  # Print the update time and version
  echo "Update time: $(date)"
  echo "Version: $(docker inspect alexj11324/orvilo1:latest | grep 'org.opencontainers.image.version' | awk -F'"' '{print $4}')"

  # Clean up unused images
  docker images | grep 'alexj11324/orvilo1' | grep -v 'latest' | awk '{print $3}' | xargs -r docker rmi > /dev/null 2>&1
  ...
  ```bash
  */5 * * * * /path/to/auto-update-orvilo.sh >> /path/to/auto-update-orvilo.log 2>&1
  ```
</Steps>
