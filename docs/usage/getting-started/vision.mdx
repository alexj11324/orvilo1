...
Orvilo supports vision capabilities — Agents can see and understand images you share. This multimodal feature enables conversations that go beyond text to include rich visual context.
...
<Callout type={'info'}>
  The image upload button only appears when you are using a vision-capable model. If you don't
...
Other providers may also offer vision models — check the deployment's model catalog for capability tags.
