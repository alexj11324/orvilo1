...
/**
 * Checks whether a slug belongs to Orvilo AI.
 *
 * Use when:
 * - Agent Signal must special-case the product-owned default assistant
 * - Code must avoid treating every virtual/builtin agent as Orvilo AI
 *
 * Expects:
 ...
 * - Orvilo AI should be managed by feature flag instead of stored chat config
 *
 * Expects:
 * - `isAgentSelfIterationFeatureEnabled` already includes the user-level feature-flag result
 *
 * Returns:
 * - `true` for Orvilo AI when the feature flag is enabled
 * - `true` for non-Orvilo AI only when `agentSelfIterationEnabled` is true
 */
