...
Use `@orvilo/prompts` formatters (`searchResultsPrompt`, `crawlResultsPrompt`, `formatTaskCreated`, etc.) to produce structured `content`. They emit XML/markdown that's already tuned for token efficiency.
...
```ts
import { BaseExecutor, type BuiltinToolContext, type BuiltinToolResult } from '@orvilo/types';
...
{
  "dependencies": {
    "@orvilo/prompts": "workspace:*"
  },
  "devDependencies": {
    "@orvilo/types": "workspace:*"
  },
  "exports": {
    ".": "./src/index.ts",
