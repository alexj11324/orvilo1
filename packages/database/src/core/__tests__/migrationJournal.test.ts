 ...
 * timestamp, as happened between 0181 and 0182 — makes a staged upgrade
 * silently skip entries: `0181` applies first, then the deploy carrying
 * `0182` sees `0182.when < max(created_at)` and never runs it, while the
...
describe('migration journal integrity', () => {
  ...
  it('pins the 0181 → 0182 → 0183 boundary that shipped the journal-order bug', () => {
    const byTag = (prefix: string) => journal.entries.find((e) => e.tag.startsWith(prefix));
    const e178 = byTag('0181');
    const e179 = byTag('0182');
    const e180 = byTag('0183');
    const e181 = byTag('0184');
    expect(e178?.tag).toBe('0181_task_workspace_claims');
    expect(e179?.tag).toBe('0182_lean_metal_master');
    expect(e180?.tag).toBe('0183_task_dispatch_origin');
    expect(e181?.tag).toBe('0184_task_workspace_claim_identity');
    ...
    // The exact regression: 0182 must sort strictly after 0181 on `when`.
  ...
  it('pins the 0182 → 0185 forward-repair pair for environments that skipped 0182', () => {
    const byTag = (prefix: string) => journal.entries.find((e) => e.tag.startsWith(prefix));
    const e179 = byTag('0182');
    const e181 = byTag('0184');
    const e182 = byTag('0185');
    // 0185 exists and carries the conditional repair — environments whose
    // recorded `created_at` already passed 0182's `when` converge through it.
    expect(e182?.tag).toBe('0185_fence_seq_forward_repair');
 ...
 * migration-journal-integrity.md` defines for 0182 (`fence_seq`):
...
const tailEntries = () => ['0181', '0182', '0183', '0184', '0185'].map((t) => journalEntry(t));
...
describe('staged upgrade replay (PGlite)', () => {
  ...
  const e178 = journalEntry('0181');
  const e179 = journalEntry('0182');
  const e180 = journalEntry('0183');
  const e181 = journalEntry('0184');
  const e182 = journalEntry('0185');

  it('M01: applies the boundary cluster in order — fence_seq arrives via 0182', async () => {
    await runScenario(async (db) => {
      // Deploy pinned at the 0181 boundary…
  ...
  it('M02: repairs a database that skipped 0182 under the old reversed journal', async () => {
    await runScenario(async (db) => {
      await pgliteMigrate(db, {
        migrationsFolder: stageMigrationsFolder([e178]),
      });
      // Deploy window where 0182's `when` still preceded 0181's but
      // 0183/0184 were already present: both apply, 0182 is skipped forever.
      await pgliteMigrate(db, {
        migrationsFolder: stageMigrationsFolder([
          e178,
          journalEntry('0182', e178.when - 1),
          e180,
          e181,
        ]),
      });
      ...
      // The repaired journal selects only 0185 — it adds the column 0182
  ...
  it('M02: a database that applied 0182 under the reversed journal needs the documented marker repair', async () => {
    await runScenario(async (db) => {
      // Fresh apply under the pre-fix journal: every entry applies in order,
      // 0182 lands with its old (earlier-than-0181) timestamp.
      const old179When = e178.when - 1;
      await pgliteMigrate(db, {
        migrationsFolder: stageMigrationsFolder([e178, journalEntry('0182', old179When)]),
      });
      expect(await fenceSeqColumn(db)).toHaveLength(1);

      // The fixed journal re-selects 0182 (its new `when` exceeds the recorded
      // boundary) and crashes on the duplicate column before it can reach
      // 0185 — no in-band migration can rescue this environment, which is why
      ...
      // Documented repair: pin the already-applied 0182 row to its corrected
  ...
  it('0185 verifies the schema definition — a divergent column fails loudly', async () => {
    const repairSql = readFileSync(
      path.join(migrationsFolder, '0185_fence_seq_forward_repair.sql'),
      'utf8',
    );
    await runScenario(async (db) => {
  ...
  it.each<{ ddl: SQL; shape: string }>([
    ...
    '0185 rejects a divergent fence_seq definition ($shape)',
    async ({ ddl }) => {
      const repairSql = readFileSync(
        path.join(migrationsFolder, '0185_fence_seq_forward_repair.sql'),
        'utf8',
