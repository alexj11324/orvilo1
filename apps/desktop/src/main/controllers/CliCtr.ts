...
export default class CliCtr extends ControllerModule {
  ...
  async runCliCommand(args: string): Promise<{ exitCode: number; stderr: string; stdout: string }> {
    ...
    const cmd = process.platform === 'win32' ? 'orvilo.cmd' : 'orvilo';
    ...
    if (remoteCtr) {
      ...
      if (token && serverUrl) {
        ...
        env.ORVILO_SERVER = normalizeServerUrl(serverUrl);
        logger.debug('Injected ORVILO_JWT / ORVILO_SERVER for CLI command');
      }
    }
