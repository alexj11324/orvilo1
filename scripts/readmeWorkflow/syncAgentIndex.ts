    ...
    .map((item) => [
      [
        genLink(
          ...
          urlJoin('https://orvilo.aspectlylabs.com/discover/assistant', item.identifier),
        ),
...
const runAgentTable = async (lang?: string) => {
  ...
  const newMd = updateReadme(
    ...
    [
      mdTable,
      `> 📊 Total agents: ${genLink(`<kbd>**${data.length}**</kbd> `, 'https://orvilo.aspectlylabs.com/discover/assistants')}`,
    ].join('\n\n'),
  );
