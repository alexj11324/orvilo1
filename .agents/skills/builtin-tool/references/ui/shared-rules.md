...
For the common "icon + file/title header, then one content box" shape, reuse `ToolResultCard` from `@orvilo/shared-tool-ui/components` instead of rebuilding it — it's already single-layer (flat wrapper, one `colorFillTertiary` content box) and is what CC `Read` / `Grep` / `Glob` / `Write` / `WebSearch` / `WebFetch` render through.
