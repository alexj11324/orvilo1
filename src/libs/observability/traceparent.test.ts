import type * as OtelApiModule from '@orvilo/observability-otel/api';
import { type Mock } from 'vitest';
import { afterEach, describe, expect, it, vi } from 'vitest';

import { getActiveTraceId, injectSpanTraceHeaders } from './traceparent';

vi.mock('@orvilo/observability-otel/api', () => {
  const inject = vi.fn();
...
describe('injectSpanTraceHeaders', () => {
  const api = vi.importMock<typeof OtelApiModule>('@orvilo/observability-otel/api');
