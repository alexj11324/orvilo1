import type { WorkspaceScanDeps } from '@orvilo/device-control';
import {
 ...
 * WorkspaceCtr
 *
 * Thin IPC layer over `@orvilo/device-control`'s workspace-scan helpers
...
export default class WorkspaceCtr extends ControllerModule {
  ...
  async listProjectSkills(params: ListProjectSkillsParams): Promise<ListProjectSkillsResult> {
    const { listProjectSkills: runListProjectSkills } =
      await import('@orvilo/device-control/workspace');
  ...
  async initWorkspace(params: InitWorkspaceParams): Promise<InitWorkspaceResult> {
    const { initWorkspace: runInitWorkspace } = await import('@orvilo/device-control/workspace');
  ...
  async statPath(params: {
    ...
    const { statPath: runStatPath } = await import('@orvilo/device-control/workspace');
