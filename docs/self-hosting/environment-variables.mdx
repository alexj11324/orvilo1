---
title: Orvilo Environment Variables - Customizing Guide
description: >-
  Learn how to customize Orvilo configuration using environment variables for
...
Orvilo provides some additional configuration options when deployed, which can be customized using environment variables.
...
If you need to override `NEXT_PUBLIC` environment variables, you can build a custom Docker image using GitHub Actions without forking the entire Orvilo repository. Here's a guide on how to do this:

1. Create a new GitHub repository for your custom build.

2. In your new repository, create a `.github/workflows` directory.

3. Inside the `.github/workflows` directory, create a file named `build-custom-orvilo.yml`:
...
env:
  REGISTRY: ghcr.io
  IMAGE_NAME: ${{ github.repository_owner }}/orvilo # Name of your image
...
6. To build your custom image, go to the "Actions" tab in your GitHub repository and manually trigger the "Build Custom Orvilo Image" workflow.

This approach allows you to create a custom build with your desired `NEXT_PUBLIC` variables without maintaining a full fork of the Orvilo repository. You can trigger a new build whenever you need to update your custom image.
