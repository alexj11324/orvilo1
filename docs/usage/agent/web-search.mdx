...
Orvilo's web search integration lets Agents access current information from the internet — real-time news, prices, documentation, and facts beyond their training cutoff. When web search is active, the Agent retrieves up-to-date answers and cites the sources it used.
...
Web search ships as a native built-in tool (`orvilo-web-browsing`) — there's nothing to install. Turn it on per Agent with one click:
...
<Callout type={'info'}>
  ...
  deployments** still need a server search provider configured (default is SearXNG
  `SEARXNG_URL` / `SEARCH_PROVIDERS`) — see [Online Search](/docs/self-hosting/advanced/online-search).
  Managed Orvilo Cloud already has this wired for you.
</Callout>
