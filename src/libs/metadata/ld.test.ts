...
import { OFFICIAL_SITE } from '@orvilo/const';
...
describe('Ld', () => {
  ...
  describe('genOrganization', () => {
    it('should generate correct organization structure', () => {
      ...
      // A ContactPoint with no address is worse than none, so it is only
      // emitted when the deployment configures a support inbox.
      if (BRANDING_EMAIL.support) {
        expect(org.contactPoint?.email).toBe(BRANDING_EMAIL.support);
      } else {
        expect(org.contactPoint).toBeUndefined();
      }
    });

    it('should point its logo at an asset the app actually serves', () => {
      const org = ld.genOrganization();

      // BRANDING_LOGO_URL is a served path; resolving it against the site origin
      // keeps the structured data absolute *and* keeps it from drifting away
      // from the logo the app itself renders.
      expect(org.image).toBe(new URL(BRANDING_LOGO_URL, OFFICIAL_SITE).href);
      expect(org.logo.url).toBe(org.image);
    });

    it('should never claim another organization’s founders', () => {
      const org = ld.genOrganization();

      // Upstream named its own founders here. Publishing that field under a
      // different organization asserts something false about real people.
      expect(org).not.toHaveProperty('founders');
    });

    it('should drop unset social accounts instead of emitting broken references', () => {
      const org = ld.genOrganization();

      // Only the accounts this deployment actually has, and no placeholder
      // entries — `null` in `sameAs` reads as a broken reference downstream.
      expect(org.sameAs).toContain(SOCIAL_URL.github);
      expect(org.sameAs).not.toContain(null);
      expect(org.sameAs).not.toContain(undefined);
      expect(org.sameAs.every((url) => typeof url === 'string')).toBe(true);
    });
  });
