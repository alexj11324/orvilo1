...
description: >-
  Guide for migrating your Orvilo deployment from NextAuth authentication to
...
This guide helps you migrate your existing NextAuth-based Orvilo deployment to Better Auth.

<Callout type={'info'}>
  Better Auth is the recommended authentication solution for Orvilo. It offers simpler configuration, more SSO providers, and better self-hosting support.
</Callout>
...
<Callout type={'error'}>
  **Important Notice**:
  ...
  - Orvilo is not responsible for any data loss or issues that may occur during the migration process
  - This guide is intended for users with development experience; not recommended for users without technical background
  - If you have any questions, feel free to ask in our [Discord](https://discord.com/invite/AYFPHvv2jT) community or [GitHub Issue](https://github.com/alexj11324/orvilo1/issues)
</Callout>
...
1. Clone the Orvilo repository and install dependencies:

   ```bash
   git clone https://github.com/alexj11324/orvilo1.git
   cd orvilo
   pnpm install
   ```
...
3. Refresh the page and try aga
   ![Clear Browser Site Data](https://hub-apac-1.objects.aspectlylabs.com/docs/733c50ee2302e5daa3d99dc0739238c8.png)
...
If you see a redirect to `signin?callbackUrl=...&error=email_not_found`, it means Orvilo cannot get the user's email.
...
When configuring SSO connections (e.g., GitHub in Auth0), make sure to enable **Email Address** permission in the Attributes section. Orvilo requires user email for authentication.
...
<Callout type={'warning'}>
  **Note for Casdoor Users**: Casdoor does not require users to have an email configured, but Orvilo strongly depends on email for authentication. If you find migration difficult due to many users without email addresses, we recommend staying on  for now. We plan to provide a self-service migration feature in the future, where users without email will be redirected to a bind-email page when they log in.
</Callout>

Solution:

1. First configure the Webhook in Orvilo to sync user data from the identity provider:
...
3. The user data will be synced to Orvilo via Webhook, and the user can then log
