    ...
    async (serialKey) => {
      ...
      const dispatch = {
        ...
        payload: {
          ...
          workflowRunId: 'legacy',
          body: { userIds: ['user-1'], topicIds: ['topic-a'] },
        },
      };
      ...
      const definition = definitions.find(
        (
          ...
          name: string;
          fn: (input: unknown, context: { retryCount: () => number }) => Promise<unknown>;
        } =>
          typeof task === 'object' &&
          task !== null &&
          'name' in task &&
          task.name === HATCHET_TASK_NAMES.workflowDispatch &&
          'fn' in task &&
          typeof task.fn === 'function',
      );
      if (!definition) throw new Error('workflow dispatch definition not registered');
      ...
      expect(mocks.enqueueHatchetTask).toHaveBeenCalledExactlyOnceWith(
        ...
        expect.objectContaining({
          dispatchId: id,
          serialKey: id,
          laneKey: createHash('sha256')
            .update('memory-user-memory.process-topic.user-1')
            .digest('hex'),
        }),
      );
