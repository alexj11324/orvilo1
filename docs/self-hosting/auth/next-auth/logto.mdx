---
title: Configuring Logto Authentication Service in Orvilo
description: >-
  Learn how to configure Logto authentication service in Orvilo, including
...
<Callout type={'warning'}>
  NextAuth (Auth.js) has been fully removed from Orvilo. The `NEXT_AUTH_SSO_PROVIDERS` and `AUTH_URL` variables shown below will now cause the deployment to fail at build/startup. This page is kept for historical reference only — use the [Better Auth provider guides](/docs/self-hosting/auth) and the [NextAuth to Better Auth migration guide](/docs/self-hosting/migration/v2/auth/nextauth-to-betterauth) instead.
</Callout>
...
<Callout type={'tip'}>
  ...
  with the Orvilo database version. In this case, Orvilo can share the same Postgres instance
  with it.
</Callout>

## Logto Configuration Process

The following assumes your Orvilo database version domain is `https://orvilo.example.com`.

If you are using a privately deployed Logto, assume its endpoint domain is `https://orvilo-auth-api.example.com`.
...
<Steps>
  ...
  Set the `Redirect URI` to `https://orvilo.example.com/api/auth/callback/logto` and the `Post sign-out redirect URI` to `https://orvilo.example.com/`.

  Set `CORS allowed origins` to `https://orvilo.example.com`.
  ...
  Configure the Logto Webhook so that Orvilo can receive notifications when user information is updated.

  Go to `Webhooks`, create a Webhook, and fill in the following fields:

  - Endpoint URL: `https://orvilo.example.com/api/webhooks/logto`
  - Events:
    - `User.Data.Updated`: Allow Orvilo to synchronize user profile information updates from Logto.
    - `User.SuspensionStatus.Updated`: Allow Orvilo to remove the active session from suspended users from logging in, only available when database session strategy is `database`.
  ...
  Set the obtained `Client ID` and `Client Secret` as `AUTH_LOGTO_ID` and `AUTH_LOGTO_SECRET` in the Orvilo environment variables.

  Configure `AUTH_LOGTO_ISSUER` in the Orvilo environment variables as follows:

  - `https://orvilo-auth-api.example.com/oidc` if you are using a privately deployed Logto
  - `https://example.logto.app/oidc` if you are using Logto Cloud

  When deploying Orvilo, you need to configure the following environment variables:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS`   | Required | Select the single sign-on provider for Orvilo. For Logto, enter `logto`.                                                                                                                 |
  ...
  | `AUTH_URL`                  | Required | This URL specifies the callback address for Auth.js during OAuth verification, needed only if the default generated redirect address is incorrect. `https://orvilo.example.com/api/auth` |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate via Logto and use Orvilo.
</Callout>
